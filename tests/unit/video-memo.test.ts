...
test("video group memoization only ignores active changes outside its own group", () => {
  ...
  const base = { group: 1, settings: {}, language: "ko", players: {}, items: [first], activeId: "outside-a" };
...
test("memo contracts deliberately ignore callback identity changes", () => {
  const groupBase = { group: 1, settings: {}, language: "ko", players: {}, items: [{ id: "first" }], activeId: "outside", onActivate: () => 1 };
