...
test("rendering registration and all three samples are required", () => {
  assert.deepEqual(targets.map(({ id }) => id), ["home", "document-result", "document-compare", "pdf-editor", "pdf-finish", "pdf-finish-structure", "pdf-page-numbers", "pdf-header-footer", "pdf-watermark", "pdf-stamp"]);
