...
const resultPath = path.join(temporaryDirectory, "preserved-result.xlsx");
...
try {
  ...
  try {
    ...
    await page.waitForSelector('.ios-switch[aria-label="XLS 수식·서식 보존"]');
    ...
    if (categories.join(",") !== "셀 내용,XLS 입력,빈 영역 정리,병합 세부 설정") {
      throw new Error(`Excel settings were not categorized as expected: ${categories.join(",")}`);
    }
    if (await page.$eval('.ios-switch[aria-label="XLS 수식·서식 보존"]', (button) => button.getAttribute("aria-checked")) !== "false") {
      throw new Error("XLS preservation was not off by default.");
    }

    await page.click('.ios-switch[aria-label="XLS 수식·서식 보존"]');
    await page.waitForFunction(() => location.pathname.endsWith("/tools/excel-merger/xls-preserve/"));
    await page.waitForSelector('.ios-switch[aria-label="XLS 수식·서식 보존"][aria-checked="true"]');
    ...
    if (officeRequests.length) throw new Error("Office assets were requested for an XLSX-only selection in preservation mode.");
    await page.reload({ waitUntil: "networkidle0" });
    ...
    await fs.writeFile(resultPath, Buffer.from(bytes));

    const merged = new ExcelJS.Workbook();
    await merged.xlsx.load(await fs.readFile(resultPath));
    ...
    if (preserved.formula !== "SUM(A1:A2)" || !preserved.bold
      || preserved.color !== "FF1F4E78" || preserved.fill !== "FFFFE699"
      || preserved.border !== "thin" || preserved.alignment !== "center"
      || preserved.numberFormat !== "0.00" || !preserved.merged) {
      throw new Error(`XLS formula or formatting was not preserved: ${JSON.stringify(preserved)}`);
    }
    const warning = await page.$eval(".result-warnings", (element) => element.textContent || "");
    if (!warning.includes("XLS 입력을 호환 XLSX 구조로 먼저 변환")) {
    ...
    await page.waitForSelector('.ios-switch[aria-label="XLS 수식·서식 보존"][aria-checked="false"]');
    ...
    console.log(`XLS preservation smoke passed: ${preparation.samples.length} progress states, ${bytes.length} output bytes.`);
  } finally {
    await browser.close();
  }
} finally {
