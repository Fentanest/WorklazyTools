...
try {
  ...
  await page.$eval(".office-app-toolbar .primary-button", (button) => button.click());
  ...
  if (!progress.samples.some((sample) => /MB/.test(sample)) || new Set(progress.samples.map((sample) => sample.split(":", 1)[0])).size < 3 || progress.samples.length > 120
    || progress.cacheEntries !== 1 || !progress.saveEnabled || !progress.canvas || progress.canvas.width < 800) {
  ...
  if (cachedMessages.length < 4 || cacheReuse.assetCount !== 6) throw new Error(`Office assets were not reused from browser storage: ${JSON.stringify(cacheReuse)}`);
  if (pageErrors.length) throw new Error(`Office browser errors:\n${pageErrors.join("\n")}`);
  console.log(`Office editor smoke passed: ${progress.samples.length} download states, ${cachedMessages.length} cached states, ${saved.length} saved bytes.`);
} finally {
  await browser.close();
