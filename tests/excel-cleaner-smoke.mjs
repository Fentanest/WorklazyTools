...
try {
  ...
  try {
    ...
    const initial = await page.evaluate(() => ({
      ...
      rules: document.querySelectorAll(".excel-cleaner-add-rule option").length,
      fileInputs: document.querySelectorAll('.excel-cleaner-page input[type="file"]').length,
    ...
    await page.click(".excel-cleaner-add-rule button");
    await page.select(".excel-cleaner-add-rule select", "collapse-spaces");
    await page.click(".excel-cleaner-add-rule button");
    await page.click('.excel-cleaner-rule:nth-child(2) button[aria-label="규칙 위로 이동"]');
    ...
    await page.evaluate(() => {
      const cards = document.querySelectorAll(".excel-cleaner-rule");
    ...
    await page.waitForFunction(() => JSON.parse(document.querySelector(".excel-cleaner-rule textarea")?.value || "{}").type === "trim-whitespace");
    order = await ruleOrder(page);
    await page.click(".excel-cleaner-actions .secondary-button");
    ...
    await page.select(".excel-cleaner-add-rule select", "regex-replace");
    await page.click(".excel-cleaner-add-rule button");
...
async function upload(page, ...paths) {
  ...
  const input = await page.$('.excel-cleaner-page input[type="file"]');
  await input.uploadFile(...paths);
  await page.waitForFunction((count) => document.querySelectorAll('[data-testid="excel-cleaner-file"]:not(:has(> p:first-of-type))').length >= count || document.querySelectorAll(".excel-cleaner-sheets").length >= count, {}, minimumReady);
  await page.waitForFunction((count) => document.querySelectorAll(".excel-cleaner-sheets").length >= count, {}, minimumReady);
}

async function ruleOrder(page) {
  return page.$$eval(".excel-cleaner-rule textarea", (items) => items.map((item) => JSON.parse(item.value).type));
}
...
async function replaceTextarea(page, selector, value) {
  ...
  await page.waitForFunction((expected) => document.querySelector(".excel-cleaner-rule textarea")?.value === expected, {}, value);
}
