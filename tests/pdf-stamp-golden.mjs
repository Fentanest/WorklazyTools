...
const stampImage = new File([PNG.sync.write(stamp)], "golden-stamp.png", { type: "image/png" });
...
await fs.writeFile(path.join(artifactDirectory, "metrics.json"), `${JSON.stringify({ combinations: comparisons, selectedPages: selectedPixels }, null, 2)}\n`);
console.log(`PDF stamp golden passed: 16 DPR/CSS-shrink/rotation coordinate combinations, 8 PDF.js/Poppler rendered pages, and selected-page placement. Artifacts: ${artifactDirectory}`);
