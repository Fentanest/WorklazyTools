...
const pages = [
  {
    ...
    sections: [
      ...
      ["비디오·오디오·이미지", "비디오를 그룹별 편집하고 오디오 파형의 선택 구간을 자르거나 음소거하며, 이미지를 개별·일괄 편집해 콜라주와 GIF로 저장합니다."],
      ["텍스트와 데이터", "텍스트 정돈과 JSON·SQL·XML 포맷, CSV·JSON·HTML 표 변환을 브라우저 안에서 실행합니다."],
      ["일정·급여·보안", "영업일·연차·글로벌 시차·급여를 계산하고 사진 속 숨은 정보(EXIF), 비밀번호와 QR 코드를 안전하게 처리합니다."],
  ...
  },
  {
    ...
    heading: "목적별로 찾는 무료 업무 도구",
    intro: "로그인이나 프로그램 설치 없이 필요한 카테고리를 고르거나 기능 이름을 검색해 바로 실행하세요.",
    sections: [
      ["문서·스프레드시트", "Excel 병합, PDF 편집·변환, Word 비교, HWP 편집과 HWP 비교 도구를 제공합니다."],
      ["이미지·영상·오디오", "이미지 일괄·개별 편집, 비디오 그룹 편집과 오디오 파형 편집을 브라우저에서 처리합니다."],
      ["텍스트·데이터", "텍스트 정돈, JSON·SQL·XML 포맷과 CSV·JSON·HTML 표 변환을 실행합니다."],
      ["일정·근무·급여", "영업일과 연차, 세계 도시 시차·회의 시간과 급여를 간편하게 계산합니다."],
      ["보안·공유", "사진 속 숨은 정보(EXIF) 제거, 안전한 비밀번호 생성·강도 확인과 QR 생성·스캔을 제공합니다."],
    ],
  },
  {
    ...
    sections: [
      ...
      ["두 가지 빈 영역 정리", "끝 여백 정리는 내용 뒤쪽의 빈 범위를 제외합니다. 중간 빈 영역 삭제는 결과 시트 전체에서 사용자가 지정한 개수 이상 연속된 빈 행 또는 빈 열 묶음을 선택적으로 삭제합니다."],
  ...
  },
  {
    ...
    sections: [
      ...
      ["자체 호스팅 실행 환경", "문서 분석 실행 파일은 Worklazy Tools와 같은 GitHub Pages 배포 경로에서 내려받으며 외부 CDN을 사용하지 않습니다. 선택한 Word 문서는 외부 서버에 업로드하지 않습니다."],
  ...
  },
  {
    ...
    sections: [
      ...
      ["자체 호스팅 한국어·영어 OCR", "스캔 페이지를 브라우저에서 인식하며 실행 파일과 언어 모델은 Worklazy Tools와 같은 GitHub Pages 배포 경로에서 제공합니다. 외부 OCR 서버나 CDN을 사용하지 않습니다."],
      ["페이지 범위와 모바일", "전체 또는 1-5, 8 같은 범위를 골라 변환할 수 있습니다. 모바일에서는 OCR 렌더 해상도를 자동 조절해 메모리 사용을 줄입니다."],
      ...
      ["대용량 처리", "대형 PDF도 처리할 수 있지만 페이지 수가 많으면 시간이 오래 걸릴 수 있습니다. 모바일에서는 필요한 페이지 범위를 먼저 지정하는 것이 좋습니다."],
    ],
  },
  {
  ...
  },
  {
    ...
    sections: [
      ...
      ["브라우저 로컬 처리", "HWP·HWPX 파일과 입력한 암호는 브라우저의 별도 작업 공간에서 처리하며 변환 서버에 업로드하지 않습니다."],
  ...
  },
  {
    ...
    sections: [
      ["영상 연속 추가와 10개 그룹", "파일 선택이나 드롭을 반복해 영상 수 제한 없이 기존 목록 뒤에 추가하고, 최대 10개 그룹에 자유롭게 배치해 출력 순서를 바꿉니다."],
      ["다시 압축하지 않는 빠른 자르기", "원본 영상을 그대로 복사해 화질 손실 없이 빠르게 구간을 저장합니다. 음성은 원본 복사·제거·호환 형식 변환 중 별도로 정하며, 시작점은 가까운 재생 기준 장면(키프레임)에 맞춰질 수 있습니다."],
      ...
      ["첫 번째 음성 제어", "영상 결과는 파일 안의 첫 번째 음성을 원본 그대로 복사하거나 완전히 제거하거나 호환 형식으로 다시 변환할 수 있습니다. 원본 음성이 출력 파일 형식과 맞지 않으면 자동 변환하지 않고 설정 변경을 안내합니다."],
      ["음질·용량 직접 설정", "비트레이트는 초당 저장할 데이터 양이고 샘플레이트는 음성을 1초에 측정하는 횟수입니다. 권장 목록뿐 아니라 직접 수치를 입력할 수 있습니다."],
      ["오디오 스튜디오에서 계속 편집", "MP3·AAC 결과 카드의 버튼을 누르면 파일을 서버나 저장소에 올리지 않고 같은 출처의 새 오디오 스튜디오 탭으로 한 번만 메모리 전달합니다."],
      ["결과별 즉시 다운로드", "각 출력이 완성될 때마다 파일별 다운로드 목록에 바로 추가합니다. 전체 개별 다운로드와 지원 브라우저의 폴더 저장을 제공하며, 공유용 ZIP은 필요할 때만 브라우저에서 따로 만듭니다."],
  ...
  {
    route: "tools/audio-studio",
    title: "온라인 오디오 파형 편집기 - MP3·WAV 자르기·음소거",
    description: "오디오 파형에서 구간을 선택해 음소거·잘라내기·복사·붙여넣기하고 WAV 또는 MP3로 저장하세요. 파일은 브라우저에서만 처리됩니다.",
    heading: "파형을 보며 오디오 샘플을 정밀하게 편집하세요.",
    intro: "MP3·WAV·M4A·AAC·OGG 파일을 브라우저에서 해석하고 파형 선택 영역에 샘플 단위 편집을 적용합니다.",
    application: "Audio Waveform Studio",
    sections: [
      ["고해상도 파형과 타임라인", "Wavesurfer.js의 파형, 밀리초 타임코드와 줌을 사용해 긴 오디오의 원하는 위치를 찾아 편집 구간을 드래그합니다."],
      ["음소거·잘라내기·삭제", "선택 구간 샘플을 0으로 바꾸거나 제거하고, 삭제 뒤의 모든 샘플을 앞으로 당겨 자연스럽게 연결합니다."],
      ["복사와 커서 붙여넣기", "선택 샘플을 현재 페이지의 메모리 클립보드에 복사하고 빨간 재생 커서가 있는 위치에 삽입합니다."],
      ["실행 취소·다시 실행", "편집 전의 비압축 음성 샘플 상태를 파일 크기에 맞는 제한 안에서 보관합니다."],
      ["WAV·MP3 내보내기", "WAV는 호환성이 높은 16비트 비압축 음성으로 만들고 MP3는 같은 GitHub Pages 배포물에 포함된 브라우저용 FFmpeg 변환기로 만듭니다."],
      ["브라우저 로컬 처리", "오디오 파일과 편집 샘플은 외부 변환 서버로 전송하지 않으며 별도 처리 작업은 결과 또는 오류 직후 종료합니다."],
      ["지원 범위", "MP3·WAV는 일반적인 최신 브라우저에서 지원합니다. M4A·AAC·OGG는 브라우저와 운영체제가 파일 안의 음성 압축 방식을 해석할 수 있어야 합니다."],
    ],
  },
  {
  ...
  },
  {
    ...
    intro: "브라우저의 별도 작업 공간에서 텍스트 문법을 분석하고 들여쓰기 또는 한 줄 형식으로 결과를 만듭니다.",
    application: "JSON SQL XML Formatter",
    sections: [["세 가지 형식", "JSON·SQL·XML 각각의 문법 검사와 정돈 규칙을 적용합니다."], ["문법 오류", "잘못된 JSON과 XML은 오류 위치와 메시지를 표시하며 원문을 임의로 복구하지 않습니다."], ["한 줄로 축소", "따옴표 안의 글자는 유지하면서 데이터 구조에 불필요한 공백과 줄바꿈을 줄입니다."]],
  },
  {
  ...
  },
  {
    ...
    sections: [["사이트에 포함된 세계지도", "Natural Earth 기반 세계지도와 도시 좌표를 사이트에 포함해 외부 지도 서버를 호출하지 않습니다."], ["정확한 시차", "국제 표준 도시 시간대(IANA) 데이터로 날짜별 서머타임과 UTC와의 시차를 반영합니다."], ["도시 비교", "지도 핀 또는 검색으로 최대 6개 도시를 선택해 현지 시각과 날짜 차이를 비교합니다."], ["회의 시간", "모든 선택 도시가 현지 09시부터 18시 사이인 30분 단위 구간을 추천합니다."]],
  },
  {
  ...
  },
  {
    ...
    intro: "사진 화면에는 보이지 않아도 파일에 함께 저장될 수 있는 GPS·기기·촬영 시각 정보를 확인하고, 사진 내용만 담은 새 사본을 만듭니다.",
    application: "Image Privacy",
    sections: [["숨은 정보(EXIF) 확인", "GPS 좌표, 촬영 기기, 촬영 시각과 소프트웨어 정보를 브라우저에서 읽습니다."], ["지원 범위", "JPG·PNG를 지원하며 HEIC·HEIF는 지원하지 않습니다. 고급 이미지 처리 기능(OffscreenCanvas)이 없는 iOS 16.3 이하에서는 제거 기능을 사용할 수 없습니다."], ["사진 내용만 새로 저장", "원본 파일 구조를 복사하지 않고 사진 내용(픽셀)만 새 JPG 또는 PNG로 저장해 부가 정보를 제거합니다."], ["브라우저 처리", "원본과 결과는 브라우저 메모리 밖으로 전송되지 않습니다."]],
  },
  {
    ...
    sections: [["운영체제 보안 난수", "운영체제가 제공하는 보안 난수 기능으로 각 문자를 고르게 선택합니다."], ["사용자 설정", "8~64자 길이와 대문자·소문자·숫자·특수문자 포함 여부를 설정합니다."], ["강도 분석", "흔한 단어와 반복 같은 패턴을 반영해 추측 난이도를 계산하고, 초당 100억 회 대입을 가정한 예상 해독 시간을 표시합니다."]],
  },
  {
    ...
    heading: "QR 코드를 만들고 카메라·사진에서 읽으세요.",
    intro: "텍스트·URL QR 코드를 만들거나 HTTPS 카메라와 업로드한 이미지의 QR 데이터를 브라우저에서 분석합니다.",
    application: "QR Studio",
    sections: [["QR 생성", "높은 오류 복원 수준과 충분한 여백으로 QR 코드를 만들고 PNG로 저장하거나 모바일 공유 시트로 보냅니다."], ["중앙 로고", "선택한 로고를 흰 여백과 함께 QR 중앙에 배치합니다."], ["실시간 카메라", "GitHub Pages의 보안 연결(HTTPS)에서 사용자 허용 후 후면 카메라 화면을 브라우저 안에서 분석하며 완료 즉시 카메라를 중지합니다."], ["사진 스캔", "브라우저 안에서 이미지를 축소하고 밝고 어두운 색을 뒤집는 방식까지 시도해 QR 데이터를 찾습니다."]],
  },
  {
    ...
    intro: "CSV·JSON·HTML 표를 서로 변환하고 결과를 복사하거나 파일로 저장합니다.",
    application: "Table Data Converter",
    sections: [["CSV", "첫 행을 열 이름으로 사용하고 따옴표·쉼표·셀 줄바꿈을 표준 CSV 규칙으로 해석합니다."], ["JSON", "객체 배열의 모든 키를 열 이름으로 모아 표 형태로 정리합니다."], ["HTML 표", "셀의 글자를 안전하게 처리해 머리글과 본문이 구분된 HTML 표를 만듭니다."], ["브라우저 처리", "표 읽기와 파일 만들기는 별도 작업 공간에서 실행하고 완료 즉시 종료합니다."]],
  },
  {
    route: "about",
  ...
  },
  {
    ...
    description: "로컬 파일 처리, Google·Naver 방문 분석, Google AdSense와 쿠키에 관한 개인정보처리방침입니다.",
    heading: "개인정보처리방침",
    intro: "시행일: 2026년 8월 14일. 작업 문서, 미디어, 입력 암호와 결과 파일은 Worklazy Tools 서버에 업로드하거나 저장하지 않습니다.",
    sections: [
      ["브라우저 로컬 처리", "Excel, Word, HWP, PDF, 비디오와 이미지 파일은 브라우저 안에서 읽고 처리합니다. 비디오의 MP3·AAC 결과를 오디오 스튜디오로 보낼 때는 같은 출처의 새 탭으로 Blob을 한 번만 전달하고 통신 채널을 닫으며 결과 파일을 브라우저 저장소에 보관하지 않습니다. 공식 rhwp 편집기는 미저장 복구를 위해 브라우저 IndexedDB에 로컬 초안을 저장할 수 있습니다."],
      ...
      ["일반 네트워크 정보", "GitHub Pages에서 사이트와 자체 호스팅 실행 파일을 불러오거나 방문 분석·광고 서비스를 사용하는 과정에는 IP 주소, 브라우저 종류와 요청 시각 같은 일반 접속 정보가 처리될 수 있습니다."],
      ["Google·Naver 방문 분석", "Google Analytics 4와 Naver Analytics로 페이지 경로와 도구 메뉴 클릭을 집계합니다. 파일 이름·내용·크기, 입력한 텍스트·급여·비밀번호, QR 결과와 상세 오류 문구는 분석 이벤트에 포함하지 않습니다."],
      ["Google AdSense", "광고가 활성화된 페이지에서는 Google과 광고 파트너가 쿠키, 웹 비콘, IP 주소와 기타 식별자를 광고 제공과 측정에 사용할 수 있습니다. 멀티스레드 실행을 위한 비디오 전용 문서에는 방문 분석과 광고 스크립트를 불러오지 않습니다."],
  ...
  },
  {
    ...
    heading: "이용약관",
    intro: "시행일: 2026년 8월 14일. Worklazy Tools는 브라우저에서 문서와 미디어 작업을 수행하는 무료 도구입니다.",
    sections: [["안전한 이용", "처리할 권한이 있는 파일만 사용하고 중요한 원본은 별도로 보관해야 합니다."], ["결과 확인", "브라우저에서 만든 문서·미디어 결과와 텍스트·QR·표 변환 결과를 실제 사용 전에 직접 확인하세요."], ["계산기 결과", "영업일·연차·급여·퇴직금은 표시된 기준과 입력값에 따른 참고 결과이며 회사 규정, 근태, 비과세 급여와 실제 신고 기준에 따라 달라질 수 있습니다."], ["보안 도구", "비밀번호 강도와 예상 해독 시간은 초당 100억 회 대입을 가정한 비교값이며 QR 스캔 결과의 URL 안전성을 보증하지 않습니다."], ["방문 분석과 광고", "서비스 개선을 위한 Google·Naver 방문 분석과 운영을 위한 Google AdSense는 개인정보처리방침과 각 제공자의 정책에 따라 처리됩니다."], ["서비스 변경", "브라우저와 서비스 제공 환경의 변경에 따라 기능이 수정되거나 일시 중단될 수 있습니다."]],
  },
  {
    route: "contact",
    title: "문의·건의·버그 제보 | Worklazy Tools",
    description: "Worklazy Tools 버그·오류 제보, 기능 제안과 개인정보 문의 방법입니다.",
    heading: "버그와 개선 의견을 알려주세요.",
    intro: "Worklazy Tools GitHub Issues를 통해 버그·오류를 제보하거나 기능을 건의할 수 있습니다. 실제 업무 문서, 개인 정보와 파일 암호는 문의 내용에 포함하지 마세요.",
    sections: [["버그·오류 제보", "브라우저 이름과 버전, 파일 형식, 선택한 옵션, 진행 로그의 마지막 단계와 오류 문구를 알려주세요."], ["기능 제안", "반복하는 파일 작업과 원하는 입력·결과 형식을 민감하지 않은 예시와 함께 설명해 주세요."]],
  },
  {
    route: "licenses",
