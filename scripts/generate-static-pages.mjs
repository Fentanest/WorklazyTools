...
const pages = [
  ...
  },
  {
    ...
    sections: [
      ...
      ["텍스트·데이터", "텍스트 정돈, JSON·SQL·XML 포맷과 CSV·JSON·HTML Table 변환을 실행합니다."],
      ["일정·근무·급여", "영업일과 연차, 세계 도시 시차·회의 시간과 급여를 간편하게 계산합니다."],
      ["보안·공유", "사진 EXIF 제거, 안전한 비밀번호 생성·강도 확인과 QR 생성·스캔을 제공합니다."],
    ],
  },
  {
  ...
  },
  {
    ...
    sections: [
      ...
      ["인코딩 없는 빠른 자르기", "원본 영상 스트림을 그대로 복사해 화질 손실 없이 빠르게 구간을 저장합니다. 오디오는 원본 유지·제거·재인코딩 중 별도로 정하며, 시작점은 키프레임 경계에 맞춰질 수 있습니다."],
      ...
      ["첫 번째 오디오 트랙 제어", "영상 결과는 첫 번째 오디오 트랙을 원본 그대로 유지하거나 완전히 제거하거나 재인코딩할 수 있습니다. 원본 유지가 출력 컨테이너와 호환되지 않으면 자동 변환하지 않고 설정 변경을 안내합니다."],
      ["비트레이트와 샘플레이트 직접입력", "영상 비트레이트와 오디오 비트레이트는 권장 목록뿐 아니라 직접 수치를 입력할 수 있습니다. 오디오 재인코딩과 MP3·AAC 추출에는 원본·44,100Hz·48,000Hz 또는 직접 샘플레이트를 적용합니다."],
      ["오디오 스튜디오에서 계속 편집", "MP3·AAC 결과 카드의 버튼을 누르면 파일을 서버나 저장소에 올리지 않고 같은 출처의 새 오디오 스튜디오 탭으로 한 번만 메모리 전달합니다."],
      ["결과별 즉시 다운로드", "각 출력이 완성될 때마다 파일별 다운로드 목록에 바로 추가합니다. 전체 개별 다운로드와 지원 브라우저의 폴더 저장을 제공하며, 공유용 ZIP은 필요할 때만 별도 Worker에서 만듭니다."],
  ...
  },
  {
    ...
    sections: [
      ...
      ["Undo·Redo", "편집 전 PCM 채널 상태를 파일 크기에 맞는 제한 안에서 보관해 실행 취소와 다시 실행을 제공합니다."],
      ["WAV·MP3 내보내기", "WAV는 16-bit PCM으로 만들고 MP3는 같은 GitHub Pages 배포물의 자체 호스팅 FFmpeg WebAssembly로 인코딩합니다."],
      ["브라우저 로컬 처리", "오디오 파일과 Float32Array 샘플은 외부 변환 서버로 전송하지 않으며 작업별 Dedicated Worker는 결과 또는 오류 직후 종료합니다."],
      ["지원 범위", "MP3·WAV는 일반적인 최신 브라우저에서 지원합니다. M4A·AAC·OGG는 브라우저와 운영체제가 해당 내부 코덱을 해석할 수 있어야 합니다."],
    ],
  },
  {
  ...
  },
  {
    ...
    sections: [["EXIF 확인", "GPS 좌표, 촬영 기기, 촬영 시각과 소프트웨어 정보를 브라우저에서 읽습니다."], ["지원 범위", "JPG·PNG를 지원하며 HEIC·HEIF는 지원하지 않습니다. OffscreenCanvas가 없는 iOS 16.3 이하에서는 제거 기능을 사용할 수 없습니다."], ["재인코딩 제거", "원본 컨테이너를 복사하지 않고 OffscreenCanvas에 픽셀을 렌더링해 새 JPG 또는 PNG로 저장합니다."], ["로컬 처리", "원본과 결과는 전용 Worker와 브라우저 메모리 밖으로 전송되지 않습니다."]],
  },
  {
  ...
  },
  {
    ...
    sections: [["QR 생성", "높은 오류 복원 수준과 충분한 여백으로 QR 코드를 만들고 PNG로 저장하거나 모바일 공유 시트로 보냅니다."], ["중앙 로고", "선택한 로고를 흰 여백과 함께 QR 중앙에 배치합니다."], ["실시간 카메라", "GitHub Pages의 HTTPS 환경에서 사용자 허용 후 후면 카메라 프레임을 Worker로 분석하며 완료 즉시 카메라를 중지합니다."], ["사진 스캔", "Worker에서 이미지를 축소하고 명암 반전을 포함해 QR 데이터를 찾습니다."]],
  },
  {
