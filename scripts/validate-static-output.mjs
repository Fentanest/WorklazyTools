...
const socialSlugByRoute = {
  ...
  "tools/pdf-editor/finish": "pdf-finish", "tools/pdf-editor/page-numbers": "pdf-page-numbers", "tools/pdf-editor/header-footer": "pdf-header-footer",
...
for (const route of routes) {
  ...
  if (["tools/excel-merger", "tools/excel-compare", "tools/excel-cleaner", "tools/document-compare", "tools/office-editor", "tools/video-studio", "tools/text-merger", "tools/qr-studio/bulk", "tools/pdf-editor/finish", "tools/pdf-editor/page-numbers", "tools/pdf-editor/header-footer"].includes(route)) {
  ...
  if (["tools/pdf-editor/finish", "tools/pdf-editor/page-numbers", "tools/pdf-editor/header-footer"].includes(route)) {
    const expectedCanonical = `https://worklazy.net/${language}/tools/pdf-editor/finish/`;
    if (!html.includes(`<link rel="canonical" href="${expectedCanonical}" />`) || !html.includes(`<meta property="og:url" content="${expectedCanonical}" />`)) {
      throw new Error(`${filePath} does not canonicalize the finish entry to ${expectedCanonical}.`);
    }
  }
}
}
...
for (const item of await fs.readdir("dist", { recursive: true, withFileTypes: true })) {
  ...
  if (!help?.includes(" hidden ") || !help.includes("data-nosnippet") || !help.includes('lang="ko"') || !help.includes('lang="en"')
    || !html.includes('event.target.type === "module"') || !html.includes('"worklazy:mounted"')) throw new Error(`${file} is missing independent startup recovery.`);
  if (/src=|href=|Worker|worker|chunk|청크|런타임|runtime|Error:|TypeError|stack/.test(help)) throw new Error(`${file} startup copy adds a request or exposes an exception.`);
  recoveryPages++;
}
