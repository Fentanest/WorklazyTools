...
function processDir(dirPath) {
    ...
    for (const entry of entries) {
        ...
        if (entry.isDirectory()) {
            ...
            if (slug === "pdfEditor") {
                 content = content.replace(/<ToolGuide[\s\S]*?(?:\/>|<\/ToolGuide>)/g, `<ToolGuideWrapper slug={mode === "convert" ? "pdfEditor.convert" : "pdfEditor.standard"} />`);
            } else if (slug === "documentRedactor") {
                 content = content.replace(/<ToolGuide[\s\S]*?>([\s\S]*?)<\/ToolGuide>/g, `<ToolGuideWrapper slug="${slug}">$1</ToolGuideWrapper>`);
                 content = content.replace(/<ToolGuide[\s\S]*?\/>/g, `<ToolGuideWrapper slug="${slug}" />`);
            } else {
                 content = content.replace(/<ToolGuide[\s\S]*?(?:\/>|<\/ToolGuide>)/g, `<ToolGuideWrapper slug="${slug}" />`);
            }
