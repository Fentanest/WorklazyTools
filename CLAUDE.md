...
- **검증 범위 설계의 주체**(「검증 범위는 변경 표면에 비례한다」·「결함 귀속」 규칙): 디스패치마다 **이번에 돌 것 / 병합 직전으로 이월할 것 / 축소 불가 4항**을 나눠 적고, **「하지 말 것」 목록**과 **기존 결함 백로그 귀속 정책**을 검수 지시서에 함께 전달한다. **이월 목록은 종결 검수에서 회수**한다 — 회수하지 않으면 유예가 누락이 된다.
...
#### 5-8. 전체 회귀를 매 라운드 돌린 것은 낭비였다 (2026-09-08)

- Excel S2 에서 CSS·핸들러 수십 줄 수정에도 시각 16장·a11y 전체·번들·`test:browser` 전체를 매번 재실행해 **라운드당 약 1시간**이 들었다. 사용자 지적("쓸데없는거 검증하는 거 아냐?")으로 범위를 좁힌 뒤 **첫 축소 검수가 34분에 새 결함 0으로 통과**했다 — 뺀 검사들이 잡을 것이 없었다는 뜻이다.
- **내가 범위를 넓힌 것도 낭비였다.** astra 가 "부모 커밋에도 동일, 새 회귀 아님"이라고 명시한 821px 결함을 진행 중인 단계에서 고치기로 했다. 사용자 신고도 아니고 그 작업과 무관했다 — **백로그로 보냈어야 한다.**
- **다만 검수 자체는 값을 했다** — 세 라운드가 각각 실제 사용자 도달 결함을 찾았고 그중 하나는 **접근성 수리가 만든 마우스 클릭 9,725px 점프**였다. 축소는 **범위**를 줄이는 것이지 **되돌림 사냥**을 줄이는 것이 아니다(「검증 범위는 변경 표면에 비례한다」 규칙의 축소 불가 4항).
- **검수 지시서 표현 주의**: "반례로 깨뜨려라"·"직접 때려라" 같은 적대적 표현은 상위 정책 필터에 걸려 **47분 진행한 검수가 중단**됐다(2026-09-08). **"우리 자신의 제품 품질 검증"임을 명시**하고 정상 입력에 대한 **응답성·정확성 측정**으로 서술한다.
