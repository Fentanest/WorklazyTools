...
- MyTradingDesk NPS JSON의 읽기 전용 export·정규화 seed, 접수번호 중복 제거와 미해결 보존, DART 목록 증분·원문 복구, 십진수 평가 계산 및 생산 workflow의 상태 복원·직렬화·후보 신선도 검사를 추가했다. 네이버 시세 재이용 권한이 확인되지 않아 생산 평가 단계는 실패하도록 막았다. — Codx
