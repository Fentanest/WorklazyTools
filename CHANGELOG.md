...
- 직접 공시의 의결권·합산 범위와 제3자 공시의 발행주식수·국민연금 단독 범위가 다를 때도 원문으로 확인한 최신 지분율을 범위를 밝혀 표에 표시한다. 비교할 수 없는 두 값의 증감·재진입 판정과 평가금액은 만들지 않는다. 수량이 가려진 문서도 같은 날 발행주식·보통주·정확한 지분율이 확인되면 비율만 반영한다. 같은 기준일의 상충 수치는 기존 자료의 기준 정보가 없어도 표에 보류로 표시한다. 게시 시각은 한국시간으로 읽기 쉽게 표시하고 표의 접수일 줄바꿈을 막았다. — Codx
- 같은 기준일의 제3자 공시 수치가 충돌해 현재값을 보류한 종목은 회사 단위의 약한 관측값이 뒤늦게 들어와도 충돌 상태와 기존 직접 공시 근거를 유지한다. 약한 관측값은 별도 확인 이력에서 볼 수 있다. — Codx
- 과거 공시 확인 범위를 짧게 요약하고 자세한 검색 구간·미확인 건수는 펼쳐 보도록 바꿨다. 직접 공시와 제3자 확인값의 현재 반영 상태를 유지하면서 한영 화면 문구를 정리했다. — Codx
