...
- PDF 직접 진입 14행의 D3 통합을 연결했다. 한·영 SEO·FAQ·소셜 12종과 정적·sitemap·리다이렉트, 재귀 AST 감사(`scripts/audit-direct-entry.mjs`)와 전체 계약 검사를 추가하고 검사기·정적 검증기에 직접 진입 범위를 등록했다. — Codx
- PDF 머리글·바닥글에 파일명 글자 수 제한(1–1000, 빈 값은 제한 없음)을 추가했다. 그래핌 단위 축약과 빈 값의 셀 폭 맞춤 보존 경로를 엔진·미리보기에 연결하고, 포함 해제 시 다른 옵션을 막지 않으며 한·영 안내와 오류를 함께 제공한다. — Codx
