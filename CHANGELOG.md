...
- 직접 공시의 의결권·합산 범위와 제3자 공시의 발행주식수·국민연금 단독 범위가 다를 때도 원문으로 확인한 최신 지분율을 범위를 밝혀 표에 표시한다. 비교할 수 없는 두 값의 증감·재진입 판정과 평가금액은 만들지 않는다. 수량이 가려진 문서도 같은 날 발행주식·보통주·정확한 지분율이 확인되면 비율만 반영한다. 게시 시각은 한국시간으로 읽기 쉽게 표시하고 표의 접수일 줄바꿈을 막았다. — Codx
