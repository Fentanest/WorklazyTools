  ...
  return (
    <main className="language-landing page-enter">
      <section className="language-landing-card glass-panel">
        ...
        <p className="mb-2 text-sm font-extrabold tracking-[.14em] text-muted-foreground">{t("landing.eyebrow")}</p>
        <h1><span>{t("landing.titlePrimary")}</span><span lang="ko">{t("landing.titleSecondary")}</span></h1>
        <p className="language-landing-description mx-auto mt-4 leading-relaxed text-muted-foreground"><span>{t("landing.descriptionPrimary")}</span><span lang="ko">{t("landing.descriptionSecondary")}</span></p>
        <div className="language-landing-actions">
          <button type="button" className={recommended === "ko" ? "recommended" : ""} aria-pressed={recommended === "ko"} onClick={() => choose("ko")}>
            <strong>{t("landing.continueKo")}</strong>
            {recommended === "ko" && <small>{t("landing.recommended")}</small>}
          </button>
          <button type="button" className={recommended === "en" ? "recommended" : ""} aria-pressed={recommended === "en"} onClick={() => choose("en")}>
