...
export const seoByPath: Record<string, SeoDefinition> = {
  ...
  "/tools/excel-compare": {
    ...
    description: "Excel·CSV 파일 쌍을 위치, 키 또는 대사 기준으로 비교하고 중복 키의 좌우 원본 행과 값·수식·서식 차이를 한 화면과 XLSX 보고서에서 확인하세요.",
    application: {
      name: "Excel Compare",
      featureList: ["XLSX·XLSM·XLS·XLSB·SpreadsheetML·CSV", "위치·키·회계 대사 비교", "중복 키 좌우 원본 행 묶음", "수식·캐시값 비교", "XLSX·XLSM 서식 비교", "쌍별 9개 시트 보고서", "다중 쌍 ZIP"],
    },
  },
  "/tools/excel-cleaner": {
...
const englishPageSeo: Record<string, SeoDefinition> = {
  ...
  "/tools/excel-compare": { title: "Excel Compare | Compare XLSX, XLS, XLSB & CSV Files", description: "Compare Excel and CSV pairs by position, keys or reconciliation rules, review each duplicate key's left and right source rows together, and download a nine-sheet XLSX report.", application: { name: "Excel Compare", featureList: ["XLSX, XLSM, XLS, XLSB, SpreadsheetML and CSV", "Position, key and reconciliation matching", "Grouped left and right rows for duplicate keys", "Formula and cached-value comparison", "XLSX and XLSM formatting comparison", "Nine-sheet report per pair", "ZIP for multiple successful pairs"] } },
