import {
  AlignLeft,
  AudioWaveform,
  Braces,
  CalendarDays,
  Clock3,
  Columns3,
  Combine,
  Copy,
  FileSearch2,
  FileEdit,
  FileSpreadsheet,
  FileStack,
  FileText,
  Files,
  Film,
  Grid3X3,
  Images,
  ImageOff,
  ImageDown,
  LockKeyhole,
  KeyRound,
  Music2,
  QrCode,
  Presentation,
  type LucideIcon,
  Rows3,
  ScanText,
  Scissors,
  Sheet,
  Sparkles,
  Table2,
  Undo2,
  WalletCards,
} from "lucide-react";

export type ToolAccent = "green" | "blue" | "violet" | "orange" | "pink" | "sky";
export type ToolCategoryId = "documents" | "media" | "text-data" | "work" | "security-share";

export interface ToolCategoryDefinition {
  id: ToolCategoryId;
  label: string;
  shortLabel: string;
  description: string;
  accent: ToolAccent;
  icon: LucideIcon;
}

export interface ToolDefinition {
  id: string;
  category: ToolCategoryId;
  path: string;
  title: string;
  shortTitle: string;
  description: string;
  eyebrow: string;
  accent: ToolAccent;
  icon: LucideIcon;
  highlights: Array<{ icon: LucideIcon; label: string }>;
  status: "available" | "soon";
}

export const toolCategories: ToolCategoryDefinition[] = [
  { id: "documents", label: "문서·스프레드시트", shortLabel: "문서", description: "문서를 편집·비교·병합하고 필요한 형식으로 변환합니다.", accent: "blue", icon: FileText },
  { id: "media", label: "이미지·영상·오디오", shortLabel: "미디어", description: "이미지와 영상, 오디오를 브라우저에서 직접 편집합니다.", accent: "pink", icon: Images },
  { id: "text-data", label: "텍스트·데이터", shortLabel: "텍스트·데이터", description: "텍스트와 코드, 표 데이터를 읽기 좋은 형태로 정리합니다.", accent: "violet", icon: Braces },
  { id: "work", label: "일정·근무·급여", shortLabel: "업무 계산", description: "업무 일정과 근무일, 시차와 급여를 간편하게 계산합니다.", accent: "green", icon: CalendarDays },
  { id: "security-share", label: "보안·공유", shortLabel: "보안·공유", description: "개인정보를 보호하고 안전한 비밀번호와 공유용 QR을 만듭니다.", accent: "orange", icon: LockKeyhole },
];

export const tools: ToolDefinition[] = [
  { id: "document-redactor", category: "security-share", path: "/tools/document-redactor", title: "개인정보 마스킹", shortTitle: "개인정보 마스킹", description: "PDF와 이미지에서 직접 선택한 영역을 검정으로 가리고 새 사본으로 저장합니다.", eyebrow: "문서 개인정보", accent: "violet", icon: LockKeyhole, highlights: [{icon: Scissors, label: "직접 영역 선택"}, {icon: Files, label: "모든 PDF 페이지 재생성"}, {icon: Images, label: "이미지 PNG 저장"}, {icon: FileSearch2, label: "결과 다시 확인"}], status: "available" },
  {
    id: "excel-merger",
    category: "documents",
    path: "/tools/excel-merger",
    title: "Excel Merger",
    shortTitle: "Excel 병합",
    description: "여러 Excel 파일과 CSV를 원하는 방식으로 하나의 파일에 정리합니다.",
    eyebrow: "스프레드시트",
    accent: "green",
    icon: FileSpreadsheet,
    highlights: [
      { icon: Sheet, label: "시트별 병합" },
      { icon: Rows3, label: "세로 병합" },
      { icon: Columns3, label: "가로 병합" },
      { icon: LockKeyhole, label: "빈 영역·암호" },
    ],
    status: "available",
  },
  {
    id: "excel-compare",
    category: "documents",
    path: "/tools/excel-compare",
    title: "Excel Compare",
    shortTitle: "Excel 비교",
    description: "Excel·CSV 파일 쌍의 값·수식·서식 차이를 위치, 키 또는 대사 기준으로 비교합니다.",
    eyebrow: "스프레드시트 비교",
    accent: "green",
    icon: FileSearch2,
    highlights: [
      { icon: Columns3, label: "위치·키 비교" },
      { icon: Rows3, label: "회계 대사" },
      { icon: FileSpreadsheet, label: "쌍별 Excel 보고서" },
      { icon: Files, label: "다중 쌍 처리" },
    ],
    status: "available",
  },
  {
    id: "excel-cleaner",
    category: "documents",
    path: "/tools/excel-cleaner",
    title: "Excel Data Cleaner",
    shortTitle: "Excel 정리",
    description: "Excel·CSV 파일에 28종 규칙을 순서대로 적용하고 새 XLSX·CSV와 처리 보고서를 만듭니다.",
    eyebrow: "스프레드시트 정리",
    accent: "green",
    icon: Table2,
    highlights: [
      { icon: Rows3, label: "구조 규칙 13종" },
      { icon: AlignLeft, label: "텍스트 규칙 7종" },
      { icon: Sparkles, label: "필터·값 변환 8종" },
      { icon: Files, label: "다중 파일·보고서" },
    ],
    status: "available",
  },
  {
    id: "document-generator",
    category: "documents",
    path: "/tools/document-generator",
    title: "DOCX 문서 일괄 생성",
    shortTitle: "문서 일괄 생성",
    description: "DOCX 양식과 Excel·CSV 데이터로 행마다 새 문서를 만듭니다.",
    eyebrow: "문서 자동 작성",
    accent: "blue",
    icon: Files,
    highlights: [
      { icon: FileText, label: "DOCX 양식 변수" },
      { icon: FileSpreadsheet, label: "Excel·CSV 여러 파일" },
      { icon: Rows3, label: "행별 DOCX 생성" },
      { icon: FileSearch2, label: "샘플·결과 보고서" },
    ],
    status: "available",
  },
  {
    id: "pdf-editor",
    category: "documents",
    path: "/tools/pdf-editor",
    title: "PDF Tools",
    shortTitle: "PDF 도구",
    description: "PDF 페이지를 편집·병합·범위별 추출하고 이미지·DOCX·XLSX·TXT로 변환하거나 브라우저 OCR을 실행합니다.",
    eyebrow: "PDF 편집·변환",
    accent: "violet",
    icon: FileStack,
    highlights: [
      { icon: Files, label: "편집·범위 추출" },
      { icon: ImageDown, label: "이미지 변환" },
      { icon: FileText, label: "DOCX·TXT" },
      { icon: FileSpreadsheet, label: "XLSX·OCR" },
    ],
    status: "available",
  },
  {
    id: "document-compare",
    category: "documents",
    path: "/tools/document-compare",
    title: "Document Compare",
    shortTitle: "문서 비교",
    description: "DOCX·DOC와 HWP·HWPX 문서의 본문·표·서식 변경을 같은 기준으로 비교합니다.",
    eyebrow: "문서 비교",
    accent: "blue",
    icon: FileSearch2,
    highlights: [
      { icon: FileSearch2, label: "Word·HWP 통합" },
      { icon: Rows3, label: "이동·분할 판정" },
      { icon: FileSpreadsheet, label: "Excel 보고서" },
      { icon: Files, label: "다중 동시 비교" },
    ],
    status: "available",
  },
  {
    id: "pdf-compare",
    category: "documents",
    path: "/tools/pdf-compare",
    title: "PDF Compare",
    shortTitle: "PDF 비교",
    description: "여러 PDF 파일 쌍의 페이지별 화면과 추출 텍스트 차이를 비교합니다.",
    eyebrow: "PDF 비교",
    accent: "violet",
    icon: FileSearch2,
    highlights: [
      { icon: Files, label: "여러 파일 쌍" },
      { icon: Grid3X3, label: "페이지별 화면 비교" },
      { icon: AlignLeft, label: "추출 텍스트 비교" },
      { icon: FileSpreadsheet, label: "쌍별 보고서" },
    ],
    status: "available",
  },
  {
    id: "hwp-editor",
    category: "documents",
    path: "/tools/hwp-editor",
    title: "HWP Editor",
    shortTitle: "HWP 편집",
    description: "HWP·HWPX 문서를 공식 rhwp Studio에서 편집하고 다시 HWP·HWPX로 저장합니다.",
    eyebrow: "한글 문서",
    accent: "orange",
    icon: ScanText,
    highlights: [
      { icon: FileText, label: "HWP·HWPX·HML" },
      { icon: ScanText, label: "전체 문서 편집" },
      { icon: Rows3, label: "서식·표·개체" },
      { icon: LockKeyhole, label: "암호 문서" },
    ],
    status: "available",
  },
  {
    id: "office-editor",
    category: "documents",
    path: "/tools/office-editor",
    title: "Browser Office Editor",
    shortTitle: "오피스 편집기",
    description: "LibreOffice 기반 화면에서 Writer·Calc·Impress 문서를 브라우저 안에서 편집하고 저장합니다.",
    eyebrow: "브라우저 오피스",
    accent: "violet",
    icon: FileEdit,
    highlights: [
      { icon: FileText, label: "Writer 문서" },
      { icon: FileSpreadsheet, label: "Calc 시트" },
      { icon: Presentation, label: "Impress 발표" },
      { icon: LockKeyhole, label: "브라우저 내 처리" },
    ],
    status: "available",
  },
  {
    id: "video-studio",
    category: "media",
    path: "/tools/video-studio/",
    title: "Video Studio",
    shortTitle: "비디오 스튜디오",
    description: "영상 수 제한 없이 최대 10개 그룹으로 편집하고 영상·오디오 스트림을 원하는 방식으로 변환합니다.",
    eyebrow: "비디오 편집",
    accent: "pink",
    icon: Film,
    highlights: [
      { icon: Film, label: "무제한 추가·10개 그룹" },
      { icon: Scissors, label: "무손실 빠른 자르기" },
      { icon: Copy, label: "그룹 간 구간 일괄 적용" },
      { icon: Files, label: "개별 저장·선택 ZIP" },
      { icon: Music2, label: "영상 속 음성 제어" },
    ],
    status: "available",
  },
  {
    id: "audio-studio",
    category: "media",
    path: "/tools/audio-studio",
    title: "Audio Studio",
    shortTitle: "오디오 스튜디오",
    description: "오디오 파형에서 구간을 편집하거나 피치·음색을 변조하고 WAV·MP3로 저장합니다.",
    eyebrow: "오디오 편집",
    accent: "violet",
    icon: AudioWaveform,
    highlights: [
      { icon: AudioWaveform, label: "고해상도 파형" },
      { icon: Scissors, label: "구간 편집" },
      { icon: Undo2, label: "음성 효과·Undo" },
      { icon: Music2, label: "WAV·MP3" },
    ],
    status: "available",
  },
  {
    id: "image-studio",
    category: "media",
    path: "/tools/image-studio",
    title: "Image Studio",
    shortTitle: "이미지 스튜디오",
    description: "선택 영역 모자이크·블러, 사진 보정과 자유 그리기를 한 편집기에서 사용하고 여러 파일은 일괄 편집·콜라주·GIF로 만듭니다.",
    eyebrow: "이미지 편집",
    accent: "sky",
    icon: Images,
    highlights: [
      { icon: Grid3X3, label: "영역 모자이크·블러" },
      { icon: Sparkles, label: "사진·그리기 통합" },
      { icon: Images, label: "일괄 리사이즈" },
      { icon: Film, label: "콜라주·GIF" },
    ],
    status: "available",
  },
  {
    id: "text-merger",
    category: "text-data",
    path: "/tools/text-merger",
    title: "Text Merger",
    shortTitle: "텍스트 병합",
    description: "직접 입력한 텍스트와 TXT 파일을 원하는 순서와 구분자로 하나로 합칩니다.",
    eyebrow: "텍스트 생산성",
    accent: "blue",
    icon: Combine,
    highlights: [
      { icon: AlignLeft, label: "직접 입력" },
      { icon: FileText, label: "TXT 불러오기" },
      { icon: Rows3, label: "통합 순서 변경" },
      { icon: Files, label: "복사·다운로드" },
    ],
    status: "available",
  },
  {
    id: "text-tools",
    category: "text-data",
    path: "/tools/text-tools",
    title: "Text Tools",
    shortTitle: "텍스트 정돈",
    description: "공백·줄바꿈을 정리하고 케이스 변환과 로컬 한국어 문장 검사를 실행합니다.",
    eyebrow: "텍스트 생산성",
    accent: "blue",
    icon: AlignLeft,
    highlights: [
      { icon: AlignLeft, label: "공백·줄 정돈" },
      { icon: Sparkles, label: "중복 줄 제거" },
      { icon: Braces, label: "Case 변환" },
      { icon: ScanText, label: "한국어 가이드" },
    ],
    status: "available",
  },
  {
    id: "text-formatter",
    category: "text-data",
    path: "/tools/text-formatter",
    title: "Code Formatter",
    shortTitle: "코드 포맷터",
    description: "JSON·SQL·XML 문법을 검사하고 들여쓰기 또는 한 줄 형식으로 정돈합니다.",
    eyebrow: "개발·데이터",
    accent: "violet",
    icon: Braces,
    highlights: [
      { icon: Braces, label: "JSON" },
      { icon: FileText, label: "SQL" },
      { icon: ScanText, label: "XML" },
      { icon: Sparkles, label: "오류 탐지" },
    ],
    status: "available",
  },
  {
    id: "work-calculator",
    category: "work",
    path: "/tools/work-calculator",
    title: "Workday Calculator",
    shortTitle: "영업일·연차",
    description: "대한민국 공휴일을 제외한 영업일과 입사일·회계연도 기준 연차를 계산합니다.",
    eyebrow: "근무 일정",
    accent: "green",
    icon: CalendarDays,
    highlights: [
      { icon: CalendarDays, label: "영업일" },
      { icon: Sparkles, label: "대체공휴일" },
      { icon: Rows3, label: "입사일 기준" },
      { icon: Columns3, label: "회계연도 기준" },
    ],
    status: "available",
  },
  {
    id: "timezone-calculator",
    category: "work",
    path: "/tools/timezone-calculator",
    title: "World Time Planner",
    shortTitle: "글로벌 시차",
    description: "세계지도에서 도시를 선택해 현지 시각과 모두가 근무 중인 글로벌 회의 시간대를 찾습니다.",
    eyebrow: "글로벌 협업",
    accent: "sky",
    icon: Clock3,
    highlights: [
      { icon: Clock3, label: "세계지도 도시 핀" },
      { icon: Sparkles, label: "서머타임" },
      { icon: Rows3, label: "근무시간 중첩" },
      { icon: FileStack, label: "44개 주요 도시" },
    ],
    status: "available",
  },
  {
    id: "payroll-calculator",
    category: "work",
    path: "/tools/payroll-calculator",
    title: "Payroll Calculator",
    shortTitle: "급여 계산",
    description: "주휴수당·월 실수령액·퇴직금을 최신 기준으로 브라우저에서 간이 계산합니다.",
    eyebrow: "급여·노무",
    accent: "orange",
    icon: WalletCards,
    highlights: [
      { icon: Clock3, label: "주휴수당" },
      { icon: WalletCards, label: "실수령액" },
      { icon: FileSpreadsheet, label: "4대 보험" },
      { icon: LockKeyhole, label: "퇴직금" },
    ],
    status: "available",
  },
  {
    id: "image-privacy",
    category: "security-share",
    path: "/tools/image-privacy",
    title: "Photo Metadata Remover",
    shortTitle: "사진 메타데이터 제거",
    description: "JPG·PNG·WebP에 숨은 GPS·촬영 기기·시각 정보(EXIF)를 확인하고 제거합니다.",
    eyebrow: "사진 메타데이터",
    accent: "pink",
    icon: ImageOff,
    highlights: [
      { icon: ScanText, label: "숨은 정보 확인" },
      { icon: ImageOff, label: "GPS 제거" },
      { icon: Images, label: "JPG·PNG·WebP" },
      { icon: LockKeyhole, label: "브라우저 처리" },
    ],
    status: "available",
  },
  {
    id: "security-tools",
    category: "security-share",
    path: "/tools/security-tools",
    title: "Password Generator",
    shortTitle: "비밀번호 생성기",
    description: "운영체제의 보안 난수로 비밀번호를 만들고 패턴 기반 추측 난이도를 확인합니다.",
    eyebrow: "보안 도구",
    accent: "violet",
    icon: KeyRound,
    highlights: [
      { icon: KeyRound, label: "보안 난수 생성" },
      { icon: LockKeyhole, label: "강도 분석" },
      { icon: Clock3, label: "해독 시간 추정" },
      { icon: Sparkles, label: "최대 64자" },
    ],
    status: "available",
  },
  {
    id: "qr-studio",
    category: "security-share",
    path: "/tools/qr-studio",
    title: "QR Studio",
    shortTitle: "QR 스튜디오",
    description: "URL·텍스트 QR을 만들거나 표 파일의 각 행을 PNG·ZIP·라벨 PDF로 일괄 생성하고 카메라·사진에서 코드를 읽습니다.",
    eyebrow: "QR 생성·일괄·스캔",
    accent: "blue",
    icon: QrCode,
    highlights: [
      { icon: QrCode, label: "QR 생성" },
      { icon: Images, label: "중앙 로고" },
      { icon: ScanText, label: "카메라·사진 스캔" },
      { icon: ImageDown, label: "일괄 PNG·라벨 PDF" },
    ],
    status: "available",
  },
  {
    id: "data-converter",
    category: "text-data",
    path: "/tools/data-converter",
    title: "Table Data Converter",
    shortTitle: "표 데이터 변환기",
    description: "CSV·JSON·HTML 표 데이터를 서로 변환하고 파일 또는 클립보드로 내보냅니다.",
    eyebrow: "표 데이터 변환",
    accent: "green",
    icon: Table2,
    highlights: [
      { icon: FileSpreadsheet, label: "CSV" },
      { icon: Braces, label: "JSON" },
      { icon: Table2, label: "HTML 표" },
      { icon: LockKeyhole, label: "브라우저 변환" },
    ],
    status: "available",
  },
];
