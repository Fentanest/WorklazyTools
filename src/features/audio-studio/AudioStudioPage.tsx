...
import { audioDirectDirty, type AudioDirectPreset } from "./audioDirect";
import { useAudioDirect } from "./audioDirectLifecycle";
export function AudioStudioPage({ preset }: { preset?: AudioDirectPreset }) {
  ...
  const pendingDecodeFile = useRef<File | undefined>(undefined);
  const inputTask = useRef<Promise<void>>(Promise.resolve());
  const resumeGeneration = useRef(0);
  useEffect(() => () => { resumeGeneration.current += 1; }, []);
  const direct = useAudioDirect(preset, audioDirectDirty({ document, undoHistory, redoHistory, busy: progress.status === "running", decoding, lastResult }), next => {
    const resume = pendingDecodeFile.current;
    const generation = ++resumeGeneration.current;
    cancelDecode();
    activeControllerRef.current?.abort();
    if (resume) void inputTask.current.then(() => { if (generation === resumeGeneration.current) void handleFilesRef.current?.([resume]); });
    terminateAudioProcessorSession();
    setDecoding(false);
    progress.reset();
    setLastResult("");
    setExportSelection(next?.purpose === "trim" && Boolean(selectionRef.current));
  });
  ...
  const commitSelection = useCallback((next: AudioSelection | undefined) => {
    ...
    setSelection(next);
    if (!next) setExportSelection(false);
  }, []);
  ...
  const acquireAudio = async (nextFiles: File[]) => {
    setAudioFailure("");
    ...
    pendingDecodeFile.current = file;
    setDecoding(true);
    ...
    try {
      ...
      if (controller.signal.aborted || !isCurrentDecode(generation)) return;
      ...
      if (direct.acceptedPreset?.purpose === "trim") setExportSelection(Boolean(initialSelection));
      ...
      if (activeControllerRef.current === controller) { activeControllerRef.current = undefined; setDecoding(false); pendingDecodeFile.current = undefined; }
    }
  };
  const handleFiles = (files: File[]) => { const task = acquireAudio(files); inputTask.current = task; return task; };
  ...
  return (
    <UtilityPage toolId="audio-studio">
      ...
      {direct.acceptedPreset && <DirectEntryNotice purpose="trim" title={language === "ko" ? "오디오 자르기" : "Trim audio"} description={language === "ko" ? "파일을 넣고 파형의 범위를 조정한 뒤 선택한 구간만 저장하세요." : "Add audio, adjust the waveform selection, and export the selected range."} />}
      ...
      {document && (
        <UtilitySectionCard step={2} title={t("audio.waveTitle")} description={t("audio.waveHelp")} className="">
          <div className="audio-waveform-toolbar mb-2 flex items-center justify-between gap-3">
            <span className="flex min-w-0 items-center gap-2 text-primary dark:text-primary"><Waves size={18} /><strong className="text-[15px] text-foreground">{t("audio.highResolution")}</strong><small className="text-[13px] text-muted-foreground">{t("audio.pxSecond", { count: ZOOM_LEVELS[zoomIndex] })}</small></span>
            <div className="flex gap-1">
              <Button className="size-9 rounded-xl bg-primary/10 text-primary hover:bg-primary/10 dark:text-primary" variant="ghost" size="icon" type="button" aria-label={t("audio.zoomOut")} disabled={zoomIndex === 0} onClick={() => updateZoom(-1)}><ZoomOut size={18} /></Button>
              <Button className="size-9 rounded-xl bg-primary/10 text-primary hover:bg-primary/10 dark:text-primary" variant="ghost" size="icon" type="button" aria-label={t("audio.zoomIn")} disabled={zoomIndex === ZOOM_LEVELS.length - 1} onClick={() => updateZoom(1)}><ZoomIn size={18} /></Button>
            </div>
          </div>
          ...
          <div className="audio-selection-panel mt-3 grid grid-cols-[minmax(160px,1fr)_minmax(180px,auto)_minmax(160px,1fr)] items-end gap-2.5 max-[620px]:grid-cols-2">
            ...
            <div className="flex min-h-[58px] flex-col items-center justify-center rounded-xl bg-primary/10 px-3 py-2 text-primary max-[620px]:col-span-2 max-[620px]:row-start-1 max-[620px]:min-h-[50px] dark:text-primary"><small className="text-xs text-muted-foreground">{t("audio.duration")}</small><strong className="mt-1 text-[15px] text-foreground tabular-nums">{formatAudioTime(selectionDuration)}</strong></div>
          ...
          <div className="audio-transport mt-3 grid grid-cols-[42px_52px_minmax(260px,1fr)_minmax(230px,auto)] items-center gap-2 rounded-2xl border border-border bg-muted p-2 max-[620px]:grid-cols-[42px_52px_minmax(0,1fr)]">
            <Button className="size-[42px] rounded-full border-border bg-card text-primary dark:text-primary" variant="outline" size="icon-lg" type="button" aria-label={t("audio.rewind")} onClick={() => wavesurferRef.current?.setTime(0)}><SkipBack size={20} /></Button>
            <Button className="audio-play-button size-[50px] rounded-full bg-primary text-white shadow-lg shadow-primary/20 hover:bg-primary/90" size="icon-lg" type="button" aria-label={isPlaying ? t("audio.pause") : t("audio.play")} aria-keyshortcuts="Space" onClick={() => void togglePlayback()}>{isPlaying ? <Pause size={22} /> : <Play size={22} />}</Button>
            ...
            <div className="audio-loop-control flex items-center gap-1 text-primary max-[620px]:col-span-3 dark:text-primary"><Repeat2 className="shrink-0" size={17} /><ToggleRow label={t("audio.loop")} checked={loop} onChange={setLoop} /></div>
          </div>
          ...
          <div className="audio-edit-toolbar mt-3 grid grid-cols-4 gap-2 max-[620px]:grid-cols-2" aria-label={t("audio.toolbar")}>
            ...
            <AudioToolButton aria-keyshortcuts="Control+Z Meta+Z" disabled={busy || !undoHistory.length} onClick={() => void restoreHistory("undo")}><Undo2 size={18} /><span>{t("audio.undo")}</span><small className="absolute top-1 right-2 grid size-[17px] place-items-center rounded-full bg-primary text-[10px] text-primary-foreground">{undoHistory.length}</small></AudioToolButton>
            <AudioToolButton aria-keyshortcuts="Control+Shift+Z Meta+Shift+Z Control+Y Meta+Y" disabled={busy || !redoHistory.length} onClick={() => void restoreHistory("redo")}><Redo2 size={18} /><span>{t("audio.redo")}</span><small className="absolute top-1 right-2 grid size-[17px] place-items-center rounded-full bg-primary text-[10px] text-primary-foreground">{redoHistory.length}</small></AudioToolButton>
          </div>
          <div className="audio-gain-control mt-3 flex items-center gap-3 rounded-xl bg-muted p-3 max-[620px]:flex-col max-[620px]:items-stretch"><label className="grid min-w-0 flex-1 grid-cols-[auto_minmax(100px,1fr)_46px] items-center gap-3 text-[13px] font-bold text-muted-foreground"><span>{t("audio.selectionGain")}</span><input className="w-full [accent-color:var(--brand)]" type="range" min={0} max={2} step={0.05} value={gain} onChange={(event) => setGain(Number(event.target.value))} /><b className="text-right text-primary tabular-nums dark:text-primary">{gain.toFixed(2)}×</b></label><Button type="button" className="rounded-xl" variant="secondary" disabled={busy || !selection} onClick={() => void applyEdit("GAIN")}>{t("audio.edit.GAIN")}</Button></div>
          ...
          />
          <div className={`audio-clipboard-status mt-2 flex items-center gap-2 rounded-xl px-2.5 py-2 text-[13px] ${clipboard ? "bg-primary/10 text-primary dark:text-primary" : "bg-muted text-muted-foreground"}`} data-has-clip={clipboard ? "true" : "false"}><ClipboardCopy size={17} /><span>{clipboard ? t("audio.clipboard", { duration: formatAudioTime(clipboard.duration), channels: clipboard.channels.length }) : t("audio.clipboardEmpty")}</span></div>
        </UtilitySectionCard>
      )}
...
function AudioToolButton({ children, ...props }: React.ComponentProps<typeof Button>) {
  return <Button {...props} type="button" variant="outline" className="relative min-h-[58px] flex-col gap-1 rounded-xl border-primary/20 bg-primary/10 p-2 text-[13px] font-bold text-primary hover:-translate-y-px hover:bg-primary/10 hover:shadow-md disabled:border-border disabled:bg-muted disabled:text-muted-foreground  dark:text-primary">{children}</Button>;
}
