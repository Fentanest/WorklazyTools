...
self.onmessage = async (event: MessageEvent<{ buffer: ArrayBuffer; type: string }>) => {
  ...
  try {
    ...
    const canvas = new OffscreenCanvas(width, height); const context = canvas.getContext("2d", { willReadFrequently: true }); if (!context) throw new Error("QR 스캔 캔버스를 만들 수 없습니다.");
  ...
  } catch (error) { self.postMessage({ type: "error", message: error instanceof Error ? error.message : "QR 코드를 읽지 못했습니다." }); }
