...
export function PayrollCalculatorPage() {
  ...
  return <div className="page tool-page page-enter utility-page payroll-page">
    ...
    {mode === "net" && <><SectionCard title="월 급여와 공제 정보"><div className="utility-form-grid"><NumberInput label="월 과세 급여" value={salary} onChange={setSalary} suffix="원" /><NumberInput label="공제대상 가족 수(본인 포함)" value={dependents} onChange={setDependents} suffix="명" /><NumberInput label="8~20세 자녀 수" value={children} onChange={setChildren} suffix="명" /></div></SectionCard><PayrollHero icon={<Calculator />} value={won.format(net.net)} label="예상 월 실수령액" /><SectionCard title="예상 공제 내역"><dl className="payroll-breakdown"><Row label="국민연금" value={net.pension} /><Row label="건강보험" value={net.health} /><Row label="장기요양보험" value={net.longTermCare} /><Row label="고용보험" value={net.employment} /><Row label="근로소득세" value={net.incomeTax} /><Row label="지방소득세" value={net.localTax} /><Row label="공제 합계" value={net.deductions} total /></dl></SectionCard></>}
    {mode === "severance" && <><SectionCard title="재직·임금 정보"><div className="utility-form-grid"><label><span>입사일</span><input type="date" value={hireDate} onChange={(event) => setHireDate(event.target.value)} /></label><label><span>퇴직일</span><input type="date" value={retirementDate} onChange={(event) => setRetirementDate(event.target.value)} /></label><NumberInput label="퇴직 전 3개월 임금 총액" value={threeMonthWages} onChange={setThreeMonthWages} suffix="원" /><NumberInput label="연간 상여금" value={annualBonus} onChange={setAnnualBonus} suffix="원" /><NumberInput label="연간 연차수당" value={annualLeavePay} onChange={setAnnualLeavePay} suffix="원" /><NumberInput label="퇴직 전 3개월 총 일수" value={periodDays} onChange={setPeriodDays} suffix="일" /></div></SectionCard><PayrollHero icon={<Landmark />} value={won.format(severance.severance)} label={severance.eligible ? `평균임금 ${won.format(severance.averageDailyWage)} · 재직 ${severance.serviceDays}일` : "1년 미만 또는 주 15시간 미만으로 법정 적용 대상 아님"} /></>}
    ...
    <ToolGuide title="간이 급여 계산 안내" description="실제 급여명세서와 법정 정산을 대체하지 않습니다." blocks={[{ title: "실수령액", paragraphs: ["비과세 급여를 제외한 월 과세급여와 부양가족 수를 기준으로 사회보험과 월 소득세를 추정합니다. 회사의 신고 기준소득월액, 비과세 항목과 원천징수 비율에 따라 달라질 수 있습니다."] }, { title: "퇴직금", paragraphs: ["퇴직 전 3개월 임금과 연간 상여·연차수당의 3/12를 반영합니다. 산출 평균임금이 통상임금보다 낮은 경우 등은 별도 확인이 필요합니다."] }]} faq={[{ question: "입력한 연봉이 저장되나요?", answer: "아니요. 모든 값은 현재 화면 상태와 브라우저 메모리에서만 계산합니다." }, { question: "세금 신고에 사용해도 되나요?", answer: "아니요. 참고용 예상치이며 실제 원천징수와 퇴직금은 회사 급여 담당자 또는 관계 기관에서 확인하세요." }]} />
  </div>;
}

function NumberInput({ label, value, onChange, suffix }: { label: string; value: number; onChange: (value: number) => void; suffix: string }) { return <label><span>{label}</span><div className="number-suffix"><input type="number" min={0} value={value} onChange={(event) => onChange(Math.max(0, Number(event.target.value)))} /><b>{suffix}</b></div></label>; }
