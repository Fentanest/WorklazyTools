 ...
 return (
  <Card className={`editor-tool-group region-effect-controls gap-0 rounded-xl bg-muted py-[11px] shadow-none ring-0${props.hasFile ? "" : " is-disabled"}`}>
   ...
   <div className="button-grid region-effect-options" data-testid="image-editor-effect-options">
    {(["mosaic", "blur"] as const).map((effect) => { const selected = props.regionEffect === effect; return <Button type="button" variant="secondary" className={cn("min-h-9 rounded-xl text-primary dark:text-primary", selected && "active bg-primary text-primary-foreground hover:bg-primary/90 dark:text-white")} aria-pressed={selected} disabled={!props.hasFile || props.regionEffectBusy} onClick={() => props.onRegionEffectChange(effect)} key={effect}>{t(`image.editor.${effect}`)}</Button>; })}
   </div>
