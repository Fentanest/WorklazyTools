...
export function GifPanel({ progress, controllerRef }: ProcessPanelProps) {
  ...
  return <UtilitySectionCard title={t("image.gif.title")} description={t("image.gif.description")}>
    ...
    {!!files.length && <div ref={frameListRef} className="gif-frame-list mt-3 grid gap-2">{files.map((file, index) => <Card className="gif-frame-row grid grid-cols-[minmax(0,1fr)_auto_auto_auto_auto] items-center gap-2 rounded-2xl border border-border bg-muted p-2.5 py-2.5 shadow-none ring-0 max-[620px]:grid-cols-[minmax(0,1fr)_auto_auto_auto]" key={`${file.name}-${file.lastModified}-${index}`}><span className="flex min-w-0 items-center gap-2 overflow-hidden text-ellipsis whitespace-nowrap"><Button type="button" className="gif-frame-drag-handle size-9 shrink-0 cursor-grab rounded-xl active:cursor-grabbing" variant="outline" size="icon" aria-label={t("image.gif.drag")}><GripVertical size={16} /></Button><b className="grid size-6 shrink-0 place-items-center rounded-lg bg-sky-500/10 text-sky-700 dark:text-sky-300">{index + 1}</b><span className="overflow-hidden text-ellipsis">{file.name}</span></span><UtilityField className="flex-row items-center gap-1.5 text-xs max-[620px]:col-span-4 max-[620px]:row-start-2"><span>{t("image.gif.frameDelay")}</span><UtilityInput className="h-9 w-[78px]" type="number" min={20} step={10} value={delays[index] ?? delay} onChange={(event) => setDelays((current) => current.map((value, itemIndex) => itemIndex === index ? Math.max(20, Number(event.target.value) || 20) : value))} /></UtilityField><Button type="button" className="size-9 rounded-xl" variant="outline" size="icon" disabled={index === 0} aria-label={t("image.gif.moveUp")} onClick={() => moveFrame(index, index - 1)}><ArrowUpToLine size={16} /></Button><Button type="button" className="size-9 rounded-xl" variant="outline" size="icon" disabled={index === files.length - 1} aria-label={t("image.gif.moveDown")} onClick={() => moveFrame(index, index + 1)}><ArrowDownToLine size={16} /></Button><Button type="button" className="size-9 rounded-xl" variant="destructive" size="icon" aria-label={t("image.gif.remove")} onClick={() => removeFrame(index)}><Trash2 size={16} /></Button></Card>)}</div>}
