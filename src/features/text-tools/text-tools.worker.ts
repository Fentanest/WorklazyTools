...
const CORE_RULES: TextRule[] = [
  ["spacing-can", "의존 명사 ‘수’는 앞말과 띄어 씁니다.", /할수(?=$|[\s,.!?])/g, "할 수"],
  ["spacing-thing", "‘것’은 앞말과 띄어 쓰는 형태인지 확인하세요.", /할것(?=$|[\s,.!?])/g, "할 것"],
  ["spacing-because", "‘때문’은 앞말과 띄어 씁니다.", /하기때문/g, "하기 때문"],
  ["spacing-no", "‘안 되다’의 부정 표현은 띄어 씁니다.", /안되(?=[는어었면지]|$)/g, "안 되"],
  ["spacing-not", "‘못 하다’의 부정 표현은 띄어 쓰는 것이 원칙입니다.", /못하(?=[는여였면지]|$)/g, "못 하"],
  ["spacing-once", "횟수를 나타내는 ‘한 번’은 띄어 씁니다.", /한번(?=\s*(?:해|보|확인|사용|시도|클릭))/g, "한 번"],
  ["spacing-several", "횟수를 나타내는 ‘몇 번’은 띄어 씁니다.", /몇번/g, "몇 번"],
  ["spacing-for", "기간을 나타내는 ‘동안’은 앞말과 띄어 씁니다.", /(일|주|개월|년)동안/g, "$1 동안"],
  ["spacing-after", "시간 순서를 나타내는 ‘후’는 앞말과 띄어 씁니다.", /(완료|작업|처리|저장|확인)후/g, "$1 후"],
  ["spacing-before", "시간 순서를 나타내는 ‘전’은 앞말과 띄어 씁니다.", /(시작|작업|처리|저장|사용)전/g, "$1 전"],
  ["typo-several", "‘며칠’의 표기를 확인하세요.", /몇일/g, "며칠"],
  ["typo-predicate", "‘어떻게’와 ‘어떡해’를 문맥에 맞게 구분하세요.", /어떻해/g, "어떡해"],
  ["typo-rate", "명사 ‘비율’의 표기를 확인하세요.", /비률/g, "비율"],
  ["typo-role", "명사 ‘역할’의 표기를 확인하세요.", /역활/g, "역할"],
  ["typo-payment", "명사 ‘결제’와 ‘결재’를 문맥에 맞게 구분하세요.", /결재(?=\s*(?:금액|수단|카드|취소|완료))/g, "결제"],
].map(([id, label, pattern, replacement]) => ({ id: id as string, label: label as string, pattern: pattern as RegExp, replacement: replacement as string }));
...
self.onmessage = (event: MessageEvent<{ type: "transform" | "inspect"; text: string; action?: TextAction }>) => {
  try {
    ...
    const findings = RULES.flatMap((rule) => {
      ...
      return [{ id: rule.id, label: rule.label, before: sample, after: sample.replace(rule.pattern, rule.replacement), count: matches.length }];
    });
    self.postMessage({ type: "inspection", findings, ruleCount: RULES.length });
  } catch (error) {
    self.postMessage({ type: "error", message: error instanceof Error ? error.message : "텍스트를 처리하지 못했습니다." });
  }
};
