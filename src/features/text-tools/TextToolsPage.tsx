...
export function TextToolsPage() {
  ...
  return (
    <UtilityPage toolId="text-tools">
      ...
      <UtilitySectionCard title={t("features:textTools.actionsTitle")} description={t("features:textTools.actionsDescription")}><div className="grid grid-cols-4 gap-2 max-[620px]:grid-cols-2" data-testid="text-actions">{ACTIONS.map((action) => <Button variant="secondary" className="h-auto min-h-[58px] justify-start whitespace-normal rounded-xl p-2.5 text-left font-bold text-primary hover:-translate-y-px" type="button" key={action} disabled={!input || busy} onClick={() => run({ type: "transform", text: input, action })}><LetterText size={17} /><span>{t(`features:textTools.actions.${action}` as never)}</span></Button>)}</div><p className="mt-3 text-[13px] font-medium leading-relaxed text-muted-foreground">{t("features:textTools.caseHelp")} <a href="#tool-guide-title" className="inline-block text-sm font-medium text-[var(--brand,var(--brand))] hover:underline dark:text-[var(--brand,var(--brand))]">처음 사용하시나요? 사용 안내 보기 &darr;</a></p></UtilitySectionCard>
