...
export function TextToolsPage() {
  ...
  return (
    <div className="page tool-page page-enter utility-page text-tools-page">
      ...
      <SectionCard title="빠른 정돈과 케이스 변환" description="원문을 바꾸지 않고 오른쪽 결과 영역에 적용합니다."><div className="utility-action-grid">{ACTIONS.map((action) => <button type="button" key={action.value} disabled={!input || busy} onClick={() => run({ type: "transform", text: input, action: action.value })}><LetterText size={17} /><span>{action.label}</span></button>)}</div><p className="term-note">camelCase는 첫 단어 뒤부터 대문자, snake_case는 밑줄(_), kebab-case는 하이픈(-)으로 영문 단어를 구분합니다. Title Case는 각 단어의 첫 글자를 대문자로 바꿉니다.</p></SectionCard>
      <SectionCard title="한국어 맞춤법·띄어쓰기 가이드" description="브라우저에 내장된 문장 규칙으로 의심 구간을 찾습니다. 문맥에 따라 원문이 맞을 수 있으므로 제안 내용을 직접 확인하세요.">
        <PrimaryButton accent="blue" disabled={!input} loading={busy} onClick={() => run({ type: "inspect", text: input })}><ScanText size={18} /> 브라우저 내장 규칙 검사</PrimaryButton>
        {ruleCount > 0 && <p className="utility-summary"><CheckCheck size={16} /> 내장 규칙 {ruleCount}개 검사 · {findings.length}개 유형 발견</p>}
