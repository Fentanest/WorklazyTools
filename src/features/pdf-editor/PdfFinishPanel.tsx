    ...
    <section className="pdf-finish-panel" data-testid="pdf-finish-ready" data-pdf-finish-tab={activeTab} data-preflight-status={preflight.status}>
      ...
      <div className="mb-2 grid grid-cols-4 gap-1" role="group" aria-label={copy.includeDecoration}>
        {(["page-numbers", "header-footer", "watermark", "stamp"] as const).map((tab) => <label key={tab} className="flex min-h-11 min-w-0 cursor-pointer items-center justify-center gap-1.5 rounded-xl text-xs font-medium sm:text-sm" data-pdf-finish-owned data-pdf-watermark-owned={tab === "watermark" || undefined} data-pdf-stamp-owned={tab === "stamp" || undefined}>
          <input type="checkbox" style={{ accentColor: "var(--primary)" }} className="h-5 w-5 shrink-0 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-primary" data-testid={`pdf-finish-include-${tab}`} aria-label={`${copy.tabs[tab]}: ${copy.includeDecoration}`} checked={enabled[tab]} disabled={locked} onChange={(event) => setDecorationEnabled(tab, event.target.checked)} />
      ...
      <div id={tabPanelId} role="tabpanel" aria-labelledby={`${tabPanelId}-${activeTab}`}>
        {stampActive && <UtilityNotice className="mb-4 flex-col" kind="warning" data-testid="pdf-stamp-notice" data-pdf-stamp-owned><strong className="block text-foreground" data-testid="pdf-stamp-notice-title">{copy.stamp.noticeTitle}</strong><span data-testid="pdf-stamp-notice-description">{copy.stamp.noticeDescription}</span></UtilityNotice>}
        <SectionCard step={1} title={copy.uploadTitle} description={copy.uploadDescription} className="">
        ...
        {file && pageCount > 0 && (
          <div className="grid grid-cols-[minmax(0,1fr)_minmax(280px,0.72fr)] items-start gap-4 max-[820px]:grid-cols-1">
            <div className="min-w-0">
              <SectionCard step={2} title={stampActive ? copy.stamp.settingsTitle : copy.settingsTitle} description={stampActive ? copy.stamp.settingsDescription : copy.settingsDescription} className="">
                {watermarkActive && <div className="grid grid-cols-3 gap-3 max-[620px]:grid-cols-1" data-pdf-watermark-owned>
                  <UtilityField>{copy.watermark.contentType}<div className="grid grid-cols-2 gap-2" role="radiogroup" aria-label={copy.watermark.contentType}><Button type="button" variant="outline" role="radio" aria-checked={watermark.content === "text"} data-testid="pdf-watermark-content-text" className={cn("min-h-11 rounded-xl", watermark.content === "text" && "border-primary bg-primary/10 text-primary ")} disabled={locked} onClick={() => updateWatermark("content", "text")}><Type size={16} />{copy.watermark.text}</Button><Button type="button" variant="outline" role="radio" aria-checked={watermark.content === "image"} data-testid="pdf-watermark-content-image" className={cn("min-h-11 rounded-xl", watermark.content === "image" && "border-primary bg-primary/10 text-primary ")} disabled={locked} onClick={() => updateWatermark("content", "image")}><ImageIcon size={16} />{copy.watermark.image}</Button></div></UtilityField>
                ...
                {!stampActive && (!watermarkActive || watermark.pattern === "single") && <fieldset className="mt-5" disabled={locked} data-pdf-watermark-owned={watermarkActive || undefined}><legend className="mb-2 text-[13px] font-bold text-muted-foreground">{copy.position}</legend><div className="grid grid-cols-3 gap-2" role="radiogroup" aria-label={copy.position}>{(watermarkActive ? watermarkRegions : regions).map((region) => <Button key={region} type="button" variant="outline" role="radio" aria-checked={form.region === region} data-finish-region={region} data-selected={form.region === region || undefined} className={cn("min-h-11 rounded-xl text-xs", form.region === region && "border-primary bg-primary/10 text-primary ")} onClick={() => updateForm("region", region)}>{copy.regions[region]}</Button>)}</div></fieldset>}
                ...
                <details className="mt-4 rounded-2xl border border-border p-4" data-testid="pdf-finish-structure" data-pdf-structure-owned>
                  ...
                  <UtilityNotice className="mt-3" kind="warning" data-testid="pdf-finish-link-preservation">{copy.structure.linkNotice}</UtilityNotice>
                  ...
                  <div className="mt-4 overflow-hidden rounded-2xl border border-border" data-testid="pdf-finish-raster" data-pdf-raster-owned>
                    ...
                    {rasterEnabled && <div className="border-t border-border p-4" data-testid="pdf-finish-raster-settings">
                      ...
                      <UtilityNotice className="mt-3" kind="warning" data-testid="pdf-finish-raster-loss">{copy.raster.lossNotice}</UtilityNotice>
              ...
              <SectionCard step={3} title={copy.pagesTitle} description={copy.pagesDescription} className="">
                ...
                {(selectionEvaluation.error || batchSelection.error) && <UtilityNotice className="mt-3" kind="error" role="alert">{copy.rangeErrors[selectionEvaluation.error || batchSelection.error]}</UtilityNotice>}
                <p className="mt-3 text-sm font-bold text-primary " aria-live="polite">{copy.selectedPages.replace("{{count}}", `${selection.exactPages.length}`).replace("{{total}}", `${pageCount}`)}</p>
            ...
            <aside className="sticky top-6 min-w-0 max-[820px]:static">
              <Card as="section" className="gap-0 overflow-visible rounded-3xl border border-border p-5 py-5 shadow-sm ring-0" aria-labelledby={`${tabPanelId}-preview-title`}>
                <div className="mb-4 flex items-center gap-2 text-primary "><SquareDashed size={18} /><h2 id={`${tabPanelId}-preview-title`} className="font-heading text-base font-medium text-foreground">{stampActive ? copy.stamp.previewTitle : copy.previewTitle}</h2></div>
                ...
                {!stampActive && <UtilityNotice className="mt-3" kind="warning" data-testid="pdf-finish-preview-disclaimer" data-pdf-watermark-owned={watermarkActive || undefined}>{copy.previewDisclaimer}</UtilityNotice>}
                {preflight.status === "checking" && <UtilityNotice className="mt-3" kind="warning" role="status" data-testid="pdf-finish-preflight-checking" data-pdf-watermark-owned={watermarkActive || undefined}>{copy.preflightChecking}</UtilityNotice>}
                {preflight.status === "ready" && <span className="sr-only" data-testid="pdf-finish-preflight-ready">ready</span>}
                {preflightErrorText && <UtilityNotice className={cn("mt-3", watermarkActive && "text-red-800 dark:text-red-200")} kind="error" role="alert" data-testid="pdf-finish-preflight-error" data-error-code={firstPreflightError?.code} data-pdf-watermark-owned={watermarkActive || undefined} data-pdf-raster-owned={firstPreflightError?.field === "raster" || undefined}>{preflightErrorText}</UtilityNotice>}
                {preflightFailure && <UtilityNotice className={cn("mt-3", watermarkActive && "text-red-800 dark:text-red-200")} kind="error" role="alert" data-testid="pdf-finish-preflight-error" data-error-code={preflight.failure} data-pdf-watermark-owned={watermarkActive || undefined} data-pdf-structure-owned={["structure-unsupported", "form-unsupported"].includes(preflight.failure) || undefined}>{preflightFailure}</UtilityNotice>}
                {!!preflight.warnings.length && <div className="mt-3 space-y-2" data-testid="pdf-finish-preflight-warnings" data-pdf-watermark-owned={watermarkActive || undefined} data-pdf-raster-owned={rasterEnabled || undefined}>{preflight.warnings.map((warning) => {
                  ...
                  return <UtilityNotice key={warning} kind="warning" data-warning-code={warning}>{message}</UtilityNotice>;
                })}</div>}
