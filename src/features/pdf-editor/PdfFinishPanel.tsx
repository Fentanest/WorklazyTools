          ...
          <div className="grid grid-cols-[minmax(0,1fr)_minmax(280px,0.72fr)] items-start gap-4 max-[820px]:grid-cols-1">
            <div className="min-w-0">
              <SectionCard step={2} title={copy.settingsTitle} description={copy.settingsDescription} className="[&_.ui-step-number]:bg-violet-700 [&_.ui-step-number]:shadow-violet-700/20">
                <UtilityField>{copy.template}<UtilityTextarea className="min-h-24 max-h-48" value={form.template} disabled={locked} maxLength={300} placeholder={copy.templatePlaceholder[activeTab]} aria-invalid={!form.template || undefined} onChange={(event) => updateForm("template", event.target.value)} /></UtilityField>
                ...
                <fieldset className="mt-5" disabled={locked}><legend className="mb-2 text-[13px] font-bold text-muted-foreground">{copy.position}</legend><div className="grid grid-cols-3 gap-2" role="radiogroup" aria-label={copy.position}>{regions.map((region) => <Button key={region} type="button" variant="outline" role="radio" aria-checked={form.region === region} data-selected={form.region === region || undefined} className={cn("min-h-11 rounded-xl text-xs", form.region === region && "border-violet-600 bg-violet-500/10 text-violet-700 dark:text-violet-300")} onClick={() => updateForm("region", region)}>{copy.regions[region]}</Button>)}</div></fieldset>
                ...
                <div className="grid grid-cols-2 gap-3 max-[620px]:grid-cols-1">
                  <UtilityField>{copy.startNumber}<UtilityInput type="number" step={1} value={startNumber} disabled={locked} onChange={(event) => { setStartNumber(Number(event.target.value)); download.clearResult(); }} /></UtilityField>
                  <UtilityField>{copy.startPage}<UtilityInput type="number" min={1} max={pageCount} step={1} value={startPage} disabled={locked} onChange={(event) => { setStartPage(Number(event.target.value)); download.clearResult(); }} /></UtilityField>
                </div>
              ...
              <SectionCard step={3} title={copy.pagesTitle} description={copy.pagesDescription} className="[&_.ui-step-number]:bg-violet-700 [&_.ui-step-number]:shadow-violet-700/20">
                <div className="grid grid-cols-[minmax(0,1fr)_180px] gap-3 max-[620px]:grid-cols-1">
                  ...
                  <UtilityField>{copy.parity}<UtilitySelect value={parity} disabled={locked} onChange={(event) => { setParity(event.target.value as PageParity); download.clearResult(); }}>{(["all", "odd", "even"] as const).map((value) => <option key={value} value={value}>{copy.parityOptions[value]}</option>)}</UtilitySelect></UtilityField>
                </div>
