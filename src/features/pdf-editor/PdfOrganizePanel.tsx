  ...
  return (
    <>
      ...
      <div className="pdf-workflow-grid grid grid-cols-[minmax(0,1fr)_290px] items-start gap-4 max-[820px]:grid-cols-1">
        <div>
          <SectionCard step={1} title={featureMessage(language, "pdf.messages.PdfOrganizePanel.addPdfs")} description={featureMessage(language, "pdf.messages.PdfOrganizePanel.loadEveryPageFromMultiplePdfsIntoOne")} className="">
              ...
              {outputMode !== "ranges" && (
                <div className="pdf-selection-toolbar mb-3 flex items-center justify-between gap-2.5 rounded-xl bg-muted p-2 max-[620px]:flex-col max-[620px]:items-stretch">
                  <div className="flex items-center gap-1 max-[620px]:justify-between">
                    <Button className="min-h-[29px] rounded-lg px-2 text-xs font-bold text-primary " variant="ghost" size="sm" type="button" onClick={selectAllPages}>{featureMessage(language, "pdf.messages.PdfOrganizePanel.selectAll")}</Button>
                    <Button className="min-h-[29px] rounded-lg px-2 text-xs font-bold text-primary " variant="ghost" size="sm" type="button" onClick={clearPageSelection}>{featureMessage(language, "pdf.messages.PdfOrganizePanel.clearSelection")}</Button>
                  ...
                  <form className="pdf-selection-range-form grid grid-cols-[minmax(0,1fr)_auto] items-center gap-1 max-[620px]:w-full" onSubmit={applySelectionRange} noValidate>
                    ...
                    <Button className="min-h-[29px] rounded-lg px-2 text-xs font-bold text-primary " variant="ghost" size="sm" type="submit">{featureMessage(language, "pdf.messages.PdfOrganizePanel.selectRange")}</Button>
          ...
          <aside ref={mobileWorkspaceRef} className="pdf-output-workspace sticky top-6 min-w-0 max-[820px]:static max-[820px]:max-h-[calc(100dvh-20px)] max-[820px]:w-[min(560px,100%)]" role={mobileWorkspace ? "dialog" : undefined} aria-modal={mobileWorkspace || undefined} aria-labelledby="pdf-output-workspace-title" onMouseDown={(event) => event.stopPropagation()}>
            <Card as="section" data-testid="pdf-output-card" className="pdf-output-card max-h-[calc(100dvh-48px)] gap-0 overflow-hidden rounded-3xl border border-border py-0 shadow-sm ring-0 max-[820px]:max-h-[calc(100dvh-20px)] max-[820px]:rounded-[28px] max-[820px]:bg-card max-[820px]:shadow-2xl">
              ...
              <div className="pdf-output-scroll min-h-0 flex-1 overflow-x-hidden overflow-y-auto px-[18px] pt-5 pb-3 [overscroll-behavior:contain] [scrollbar-color:rgba(118,118,128,.38)_transparent] [scrollbar-gutter:stable] [scrollbar-width:thin] max-[820px]:pt-2">
                <div className="flex items-center gap-2 text-primary "><Layers3 size={18} /><h2 className="font-heading text-[15px] font-medium text-foreground" id="pdf-output-workspace-title">{featureMessage(language, "pdf.messages.PdfOrganizePanel.editOutputSummary")}</h2></div>
                <div className="pdf-output-mode-list mt-4 grid gap-1.5" role="radiogroup" aria-label={featureMessage(language, "pdf.messages.PdfOrganizePanel.pdfOutputMode")}>
                  ...
                  ] as const).map(([value, Icon, label, hint]) => { const selected = outputMode === value; return <Button key={value} type="button" role="radio" aria-checked={selected} data-selected={selected || undefined} variant="outline" className={cn("min-h-[54px] w-full grid-cols-[auto_minmax(0,1fr)] justify-start gap-2 rounded-xl border-transparent bg-muted px-2.5 py-2 text-left text-muted-foreground", selected && "border-primary bg-primary/10 text-primary hover:bg-primary/15 ")} disabled={locked} onClick={() => setMode(value)}><Icon size={16} /><span className="flex min-w-0 flex-col"><strong className="text-[13px] text-foreground">{label}</strong><small className="mt-1 whitespace-normal text-xs leading-snug text-muted-foreground">{hint}</small></span></Button>; })}
                </div>

                {outputMode === "ranges" && (
                  <section className="pdf-multi-range-panel mx-[-7px] mt-4 mb-1 rounded-2xl border border-primary/20 bg-primary/10 p-2.5" aria-label={featureMessage(language, "pdf.messages.PdfOrganizePanel.outputRanges")}>
                    ...
                    {quickSplitOpen ? (
                      <div className="pdf-range-selection-toolbar grid grid-cols-[minmax(0,1fr)_auto] items-center gap-2 rounded-xl border border-primary/20 bg-card p-2 text-xs text-muted-foreground" data-mode="quick-split" role="region" aria-label={featureMessage(language, "pdf.messages.PdfOrganizePanel.quickContinuousSplit")}>
                        ...
                        <b className="text-primary ">{featureMessage(language, "pdf.messages.PdfOrganizePanel.outputCount", { p0: splitAfterPageIds.size + 1 })}</b>
                        <div className="pdf-range-toolbar-actions flex justify-end gap-1"><Button type="button" size="xs" variant="ghost" onClick={cancelQuickSplit}>{featureMessage(language, "pdf.messages.PdfOrganizePanel.cancel")}</Button><Button type="button" size="xs" className="bg-primary text-primary-foreground hover:bg-primary/90" onClick={applyQuickSplit}>{featureMessage(language, "pdf.messages.PdfOrganizePanel.applyContinuousSplit")}</Button></div>
                      </div>
                    ) : (
                      <div className="pdf-range-selection-toolbar grid grid-cols-[minmax(0,1fr)_auto] items-center gap-2 rounded-xl border border-primary/20 bg-card p-2 text-xs text-muted-foreground" role="region" aria-label={featureMessage(language, "pdf.messages.PdfOrganizePanel.visualRangeSelection")}>
                        ...
                        <strong className="text-primary ">{featureMessage(language, "pdf.messages.PdfOrganizePanel.pagesChecked", { p0: activeRangeIndexes.size, p1: pages.length })}</strong>
                    ...
                    <div className="pdf-range-groups grid max-h-[min(36dvh,330px)] gap-2 overflow-y-auto pr-1 [overscroll-behavior:contain] [scrollbar-gutter:stable]">
                      {evaluatedRanges.map((row, index) => { const active = row.id === activeRange?.id; return <div className={cn("pdf-range-group grid grid-cols-[minmax(0,1fr)_32px] items-center gap-2 rounded-xl border border-transparent bg-card p-2", active && "border-primary ring-2 ring-primary/20", row.error && "border-destructive/30")} data-active={active || undefined} data-invalid={row.error ? "true" : undefined} key={row.id}>
                        <Button type="button" className="pdf-range-activate col-start-1 row-start-1 w-fit justify-self-start rounded-lg p-1 text-[10px] font-bold text-muted-foreground" variant="ghost" size="sm" onClick={() => setActiveRangeId(row.id)} aria-pressed={active} disabled={locked || quickSplitOpen}><b className="grid size-[25px] place-items-center rounded-lg bg-primary text-[13px] text-primary-foreground">{index + 1}</b><span>{featureMessage(language, active ? "pdf.messages.PdfOrganizePanel.editing" : "pdf.messages.PdfOrganizePanel.editWithChecks")}</span></Button>
                    ...
                    <p className="mt-2.5 text-xs leading-relaxed text-muted-foreground">{featureMessage(language, "pdf.messages.PdfOrganizePanel.pageNumbersFollowTheCurrentEditorOrderFor")} <code className="rounded bg-primary/10 px-1 py-0.5 text-primary ">5, 1-3</code>{featureMessage(language, "pdf.messages.PdfOrganizePanel.createsAPdfOrdered5123")}</p>
                  </section>
                )}
              ...
              <div className="pdf-output-action-zone max-h-[min(58dvh,540px)] shrink-0 overflow-y-auto border-t border-border bg-primary/5 px-[18px] pt-3.5 pb-[18px] [scrollbar-gutter:stable] max-[820px]:pb-[calc(18px+env(safe-area-inset-bottom))]">
                ...
                <OperationProgress {...operation} compact accent="coral" title={featureMessage(language, "pdf.messages.PdfOrganizePanel.pdfEditExtractLog")} />
      ...
      {mobileWorkspace && (pages.length > 0 || operation.status === "running" || !!download.result) && (
        <div className="pdf-mobile-output-dock fixed right-3 bottom-[calc(82px+env(safe-area-inset-bottom))] left-3 z-50 grid min-h-[62px] grid-cols-[minmax(0,1fr)_auto] items-center gap-2 rounded-[19px] border border-border bg-card/95 p-2 shadow-xl backdrop-blur-xl" role="region" aria-label={featureMessage(language, "pdf.messages.PdfOrganizePanel.outputWorkspace")}>
          <Button ref={mobileWorkspaceTriggerRef} className="pdf-mobile-output-summary min-h-[47px] min-w-0 justify-start gap-2 rounded-xl px-2 text-left text-primary " variant="ghost" type="button" aria-haspopup="dialog" aria-expanded={mobileWorkspaceOpen} onClick={openMobileOutputWorkspace}>
            <Settings2 size={18} />
          ...
          {download.result && operation.status === "success" ? (
            <a className="pdf-mobile-download inline-flex min-h-[46px] min-w-24 items-center justify-center gap-1 rounded-xl bg-primary px-3 text-center text-xs font-bold leading-tight text-primary-foreground shadow-md shadow-primary/20" href={download.result.url} download={download.result.fileName}><Download size={17} /> {featureMessage(language, "pdf.messages.PdfOrganizePanel.downloadResult")}</a>
          ) : (
            <Button className="pdf-mobile-create min-h-[46px] min-w-24 rounded-xl bg-primary px-3 text-center text-xs font-bold leading-tight text-primary-foreground shadow-md shadow-primary/20 hover:bg-primary/90" type="button" disabled={exportDisabled} onClick={() => void exportPdf()}>{operation.status === "running" ? `${operation.progress}%` : operation.status === "error" ? featureMessage(language, "pdf.messages.PdfOrganizePanel.retry") : createLabel}</Button>
          )}
        </div>
      )}
    </>
