    ...
    >
      ...
      {(onRotate || onRemove || onMove) && (
        <div className="pdf-page-actions grid grid-cols-[29px_1fr_29px_31px] gap-1 border-t border-border p-2">
          ...
          {onRotate && <Button className="min-h-[29px] rounded-lg bg-violet-500/10 p-0 text-xs font-bold text-violet-700 hover:bg-violet-500/20 dark:text-violet-300" variant="ghost" size="sm" type="button" data-page-action="rotate" onClick={onRotate} aria-label={featureMessage(language, "pdf.messages.PdfThumbnail.rotate90DegreesClockwise")}><RotateCw size={15} /><span>{featureMessage(language, "pdf.messages.PdfThumbnail.rotate")}</span></Button>}
