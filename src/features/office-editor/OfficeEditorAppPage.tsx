...
export function OfficeEditorAppPage() {
 ...
 useEffect(() => {
  ...
  void takePendingOfficeFile()
   .then((pendingFile) => { if (pendingFile) chooseFile(pendingFile); else void start(undefined); })
   .catch(() => {
 ...
 >
   {state !== "editing" && <div className="absolute inset-0 z-[2] grid place-items-center content-center gap-2 bg-[radial-gradient(circle_at_50%_35%,#47474f,#29292e_68%)] p-6 text-center text-[#d7d7dc] [&_.ui-drop-zone-wrap]:w-[min(720px,100%)] [&_svg]:text-[#b496ff]">
    {state === "idle" || (state === "error" && !file) ? <>
     <FileDropZone files={[]} onFiles={(files) => { const selected = files.at(-1); if (selected) chooseFile(selected); }} accept={OFFICE_ACCEPT} hint={L("문서를 선택해 주세요. 파일을 놓으면 자동으로 엽니다.", "Choose a document. Drop a file to open it automatically.")} accent="coral" disabled={busy} />
   ...
   <canvas ref={canvasRef} id="qtcanvas" contentEditable tabIndex={0} className="block h-full min-h-[inherit] w-full border-0 p-0 outline-0 focus-visible:ring-3 focus-visible:ring-primary/50" data-testid="office-canvas" onPointerDown={(event) => event.currentTarget.focus()} onContextMenu={(event) => event.preventDefault()} />
  </div>
