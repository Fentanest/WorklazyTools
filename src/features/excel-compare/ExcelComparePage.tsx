...
const STATUS_DOT_CLASSES: Record<ExcelCompareStatus, string> = {
 ...
 removed: "bg-rose-500",
 duplicate: "bg-fuchsia-600",
 ambiguous: "bg-fuchsia-600",
 ...
 return (
  <UtilityPage toolId="excel-compare">
   ...
   <UtilitySectionCard step={1} title={t("features:excelCompare.pairs.title")} description={t("features:excelCompare.pairs.description")}>
    ...
    <Button className="mt-3 min-h-11 self-start rounded-xl border-primary/50 px-4 font-bold text-primary shadow-sm hover:border-primary! hover:bg-primary/10! focus-visible:border-primary! focus-visible:ring-primary/30! dark:border-primary/60 dark:text-primary dark:hover:border-primary! dark:hover:bg-primary/10!" data-testid="excel-add-pair" variant="outline" type="button" onClick={addPair} disabled={operation.status === "running"}><Plus size={17} /> {t("features:excelCompare.pairs.add")}</Button>
   </UtilitySectionCard>
   ...
   <UtilitySectionCard step={2} title={t("features:excelCompare.method.title")} description={t("features:excelCompare.method.description")}>
    <div className="grid grid-cols-3 gap-2 max-[720px]:grid-cols-1" data-testid="excel-compare-mode-grid" role="radiogroup" aria-label={t("features:excelCompare.method.title")}>
     {(["position", "key", "reconcile"] as ExcelCompareMode[]).map((value) => { const selected = mode === value; return <button type="button" role="radio" aria-checked={selected} data-selected={selected || undefined} className={`min-h-20 rounded-2xl border p-3 text-left outline-none transition-[border-color,background-color,box-shadow,transform] hover:-translate-y-0.5 hover:border-primary/60 focus-visible:ring-3 focus-visible:ring-primary/30 dark:hover:border-primary/70 ${selected ? "border-primary bg-primary/5 text-foreground ring-2 ring-primary/25 dark:border-primary dark:bg-primary/70 dark:ring-primary/30" : "border-border bg-muted/45 text-foreground"}`} key={value} onClick={() => setMode(value)}><strong className="block text-sm">{t(`features:excelCompare.method.${value}` as never)}</strong><small className="mt-1 block text-xs leading-relaxed text-muted-foreground">{t(`features:excelCompare.method.${value}Description` as never)}</small></button>; })}
    </div>
   </UtilitySectionCard>
   ...
   {(completed.length> 0 || failed.length> 0) && <Card as="section" className="mt-4 gap-0 overflow-visible rounded-3xl border border-border p-4 shadow-sm" data-testid="excel-compare-results" aria-labelledby="excel-compare-results-title" onFocusCapture={(event) => scheduleResultFocusVisibility(event.target)}>
    <div><p className="text-xs font-extrabold tracking-[.08em] text-primary uppercase dark:text-primary">RESULTS</p><h2 className="mt-1 font-heading text-xl font-medium" id="excel-compare-results-title">{t("features:excelCompare.results.title")}</h2><p className="mt-2 text-sm text-muted-foreground">{t("features:excelCompare.results.description", { success: completed.length, failed: failed.length })} <a href="#tool-guide-title" className="inline-block text-sm font-medium text-[var(--brand,theme(colors.blue.600))] hover:underline dark:text-[var(--brand,theme(colors.blue.400))]">처음 사용하시나요? 사용 안내 보기 &darr;</a></p></div>
    ...
    {failed.map((item) => <UtilityNotice className="mt-2" data-testid="excel-compare-error" kind="error" role="alert" key={`${item.pairId}-${item.leftName}`}><span className="flex flex-col"><strong>{item.leftName && item.rightName ? `${item.leftName} ↔ ${item.rightName}` : t("features:excelCompare.results.zip")}</strong>{item.message}</span></UtilityNotice>)}
    {duplicateCount> 0 && <UtilityNotice className="mt-3" data-testid="excel-duplicate-guidance"><span><strong className="block">{t("features:excelCompare.results.duplicateCount", { count: duplicateCount })}</strong><span className="mt-1 block text-sm">{t("features:excelCompare.results.duplicateGuidance")}</span></span></UtilityNotice>}
...
function PairCard({ pair, index, mode, busy, t, updatePair, selectFile, changeSheet, changeHeaderInput, commitHeader, removePair, canRemove }: {
 ...
 return <Card as="article" className="gap-0 overflow-visible rounded-3xl border border-border p-4 shadow-sm" data-testid="excel-compare-pair">
  <div className="grid grid-cols-[auto_minmax(0,1fr)_auto] items-center gap-2.5"><span className="grid size-8 place-items-center rounded-full bg-primary text-sm font-extrabold text-white">{index + 1}</span><div className="min-w-0"><strong className="block text-sm">{t("features:excelCompare.pairs.pair", { number: index + 1 })}</strong><small className="mt-0.5 block text-xs text-muted-foreground">{t("features:excelCompare.pairs.pairHint")}</small></div><div className="flex items-center gap-1.5"><Button className="size-11 rounded-xl border-primary/60 text-primary shadow-sm hover:border-primary! hover:bg-primary/10! focus-visible:border-primary! focus-visible:ring-primary/30! dark:border-primary/70 dark:text-primary dark:hover:border-primary! dark:hover:bg-primary/10!" data-testid="excel-pair-swap" variant="outline" size="icon-lg" type="button" disabled={busy || inspectionBusy} onClick={() => updatePair(pair.id, swapPairSides)} aria-label={t("features:excelCompare.pairs.swap")}><ArrowLeftRight size={19} /></Button>{canRemove && <Button className="size-11 rounded-xl text-destructive hover:bg-destructive/10 hover:text-destructive" variant="ghost" size="icon-lg" type="button" disabled={busy} onClick={() => removePair(pair.id)} aria-label={t("features:excelCompare.pairs.remove", { number: index + 1 })}><Trash2 size={18} /></Button>}</div></div>
  ...
  {pair.unassignedFileCount> 0 && <UtilityNotice className="mt-2" data-testid="excel-pair-overflow" role="status"> {t("features:excelCompare.pairs.overflow", { count: pair.unassignedFileCount })}</UtilityNotice>}
  <div className="mt-3 grid grid-cols-2 gap-3 max-[720px]:grid-cols-1" data-testid="excel-pair-files">
   {(["left", "right"] as const).map((side) => {
    ...
    {pair[side] && <div className="grid grid-cols-[auto_minmax(0,1fr)_auto] items-center gap-2 rounded-xl bg-background p-2 ring-1 ring-border" data-testid="excel-selected-file"><FileSpreadsheet className="text-primary dark:text-primary" size={16} /><span className="min-w-0"><strong className="block overflow-hidden text-ellipsis whitespace-nowrap text-sm">{pair[side]!.name}</strong><small className="block text-xs text-muted-foreground">{formatBytes(pair[side]!.size)}</small></span><Button type="button" variant="ghost" size="icon-sm" className="rounded-lg text-destructive hover:bg-destructive/10 hover:text-destructive" onClick={() => void selectFile(pair.id, side, undefined)} aria-label={t("common:files.remove", { name: pair[side]!.name })}><X size={15} /></Button></div>}
    {pair[`${side}Error`] && <p className="mt-2 flex items-start gap-1.5 text-sm font-bold text-destructive" data-testid="excel-file-error"> {pair[`${side}Error`]}</p>}
