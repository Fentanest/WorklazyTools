...
export function WorkCalculatorPage() {
  ...
  try { leave = calculateAnnualLeave(hire, asOf, method, language); } catch { leave = undefined; }
  ...
  return <div className="page tool-page page-enter utility-page work-calculator-page">
    ...
    {mode === "business" ? <>
      ...
      {leave && <SectionCard title={t("work.leaveResult")}><div className="hero-result"><Calculator size={28} /><div><strong>{t("work.days", { count: leave.days.toLocaleString(i18n.language) })}</strong><p>{leave.detail}</p></div></div><p className="legal-note">{t("work.leaveNote")}</p></SectionCard>}
    </>}
