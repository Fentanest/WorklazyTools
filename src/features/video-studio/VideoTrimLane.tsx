...
export const VideoTrimLane = memo(function VideoTrimLane({ item, index, active, groupSize, language, onActivate, onStart, onEnd, onBoundary, onPlay, onNudge, onApplyGroup }: {
  ...
  return (
    ...
    >
      ...
      <div className="video-range-values mt-0.5 grid grid-cols-[auto_minmax(90px,1fr)_auto] items-end gap-2.5 max-[620px]:grid-cols-[1fr_auto_1fr]">
        <label className="flex items-center gap-1.5 text-[13px] font-bold text-muted-foreground max-[620px]:flex-col max-[620px]:items-start" data-trim-boundary="start"><span>{featureMessage(language, "video.messages.VideoTrimLane.start")}</span><div className="video-boundary-stepper inline-flex items-center gap-[3px]"><Button type="button" variant="secondary" size="icon-xs" className="h-[29px] w-[27px] rounded-lg text-pink-700 dark:text-pink-300" aria-label={featureMessage(language, "video.messages.VideoTrimLane.moveStartEarlier", { p0: stepLabel })} aria-keyshortcuts="Alt+ArrowLeft" onClick={() => adjustBoundary("start", -1)}><ChevronLeft size={13} /></Button><TrimNumberInput value={item.start} min={0} max={item.end - 0.05} onCommit={(value) => onStart(value, false)} /><Button type="button" variant="secondary" size="icon-xs" className="h-[29px] w-[27px] rounded-lg text-pink-700 dark:text-pink-300" aria-label={featureMessage(language, "video.messages.VideoTrimLane.moveStartLater", { p0: stepLabel })} aria-keyshortcuts="Alt+ArrowRight" onClick={() => adjustBoundary("start", 1)}><ChevronRight size={13} /></Button></div></label>
        <small className="self-center text-center text-[13px] text-muted-foreground">{featureMessage(language, "video.messages.VideoTrimLane.selection")} {formatTime(item.end - item.start)}</small>
        <label className="flex items-center gap-1.5 text-[13px] font-bold text-muted-foreground max-[620px]:items-end max-[620px]:justify-end max-[620px]:flex-col" data-trim-boundary="end"><span>{featureMessage(language, "video.messages.VideoTrimLane.end")}</span><div className="video-boundary-stepper inline-flex items-center gap-[3px]"><Button type="button" variant="secondary" size="icon-xs" className="h-[29px] w-[27px] rounded-lg text-pink-700 dark:text-pink-300" aria-label={featureMessage(language, "video.messages.VideoTrimLane.moveEndEarlier", { p0: stepLabel })} aria-keyshortcuts="Alt+Shift+ArrowLeft" onClick={() => adjustBoundary("end", -1)}><ChevronLeft size={13} /></Button><TrimNumberInput value={item.end} min={item.start + 0.05} max={item.duration} onCommit={(value) => onEnd(value, false)} /><Button type="button" variant="secondary" size="icon-xs" className="h-[29px] w-[27px] rounded-lg text-pink-700 dark:text-pink-300" aria-label={featureMessage(language, "video.messages.VideoTrimLane.moveEndLater", { p0: stepLabel })} aria-keyshortcuts="Alt+Shift+ArrowRight" onClick={() => adjustBoundary("end", 1)}><ChevronRight size={13} /></Button></div></label>
      </div>
