...
export function ImagePrivacyPage() {
  ...
  const execute = async () => {
    ...
    const buffer = await file.arrayBuffer(); worker.postMessage({ name: file.name, type: file.type, buffer, language: i18n.language }, [buffer]);
  };
