...
export function ExcelCleanerPage() {
  ...
  return <UtilityPage toolId="excel-cleaner">
    <div className="contents" data-testid="excel-cleaner-page">
      ...
      <Card className="mt-4 flex-row items-center justify-end gap-2 overflow-visible rounded-3xl border border-border p-4 shadow-md max-[620px]:grid max-[620px]:grid-cols-2 [&>*]:min-h-10" data-testid="excel-cleaner-actions"><Button type="button" variant="secondary" className="rounded-xl font-bold" onClick={() => void runPreview()} disabled={!firstReady || !parsedPipeline.pipeline || busy}><RefreshCw size={17} />{t("features:excelCleaner.actions.preview")}</Button><Button type="button" size="lg" data-ui-component="primary-button" className="h-12 rounded-2xl bg-primary text-[15px] font-bold text-white shadow-md shadow-primary/20 hover:bg-primary/90 focus-visible:border-primary focus-visible:ring-primary/30" onClick={() => void run()} disabled={!readyItems.length || !parsedPipeline.pipeline || busy} aria-busy={busy}>{busy && <LoaderCircle className="animate-spin" size={19} aria-hidden="true" />}<FileSpreadsheet size={18} />{t("features:excelCleaner.actions.run", { count: readyItems.length })}</Button></Card>
