...
import { AlertCircle, ArrowDown, ArrowUp, Download, FileSpreadsheet, GripVertical, LoaderCircle, Plus, RefreshCw, Trash2, X } from "lucide-react";
...
export function ExcelCleanerPage() {
  ...
  return <UtilityPage toolId="excel-cleaner">
    <div className="contents" data-testid="excel-cleaner-page">
      <PageHeader eyebrow="EXCEL CLEANER" title={t("features:excelCleaner.title")} description={t("features:excelCleaner.description")} />

      <UtilitySectionCard step={1} title={t("features:excelCleaner.files.title")} description={t("features:excelCleaner.files.description")}>
        ...
        <div className="mt-3 grid gap-2.5" data-testid="excel-cleaner-files">
          {items.map((item, index) => <Card as="article" className="gap-0 overflow-visible rounded-2xl border border-border p-3 shadow-sm" key={item.id} data-testid="excel-cleaner-file" data-inspecting={item.inspecting || undefined}>
            <header className="grid grid-cols-[auto_minmax(0,1fr)_auto] items-center gap-2.5"><FileSpreadsheet className="text-green-700 dark:text-green-300" size={18} /><span className="min-w-0"><strong className="block overflow-hidden text-ellipsis whitespace-nowrap text-sm">{item.file.name}</strong><small className="mt-0.5 block text-xs text-muted-foreground">{formatBytes(item.file.size)}{item.inspection ? ` · ${item.inspection.format.toUpperCase()}` : ""}</small></span><Button type="button" variant="ghost" size="icon-sm" className="rounded-lg text-destructive hover:bg-destructive/10 hover:text-destructive" data-testid="excel-cleaner-remove" aria-label={t("common:files.remove", { name: item.file.name })} onClick={() => void replaceFiles(items.filter((_candidate, position) => position !== index).map((candidate) => candidate.file))}><X size={16} /></Button></header>
            {item.inspecting && <p className="mt-2 text-sm text-muted-foreground">{t("features:excelCleaner.files.inspecting")} <a href="#tool-guide-title" className="inline-block text-sm font-medium text-[var(--brand,theme(colors.blue.600))] hover:underline dark:text-[var(--brand,theme(colors.blue.400))]">처음 사용하시나요? 사용 안내 보기 &darr;</a></p>}
            {item.error && <UtilityNotice className="mt-2" tone="error" role="alert"><AlertCircle className="mt-0.5 shrink-0" size={16} /><span>{item.error}</span></UtilityNotice>}
            {item.inspection?.softLimitExceeded && !item.inspection.hardLimitExceeded && <UtilityNotice className="mt-2"><AlertCircle className="mt-0.5 shrink-0" size={16} /><span>{t("features:excelCleaner.files.large", { count: item.inspection.cellCount.toLocaleString() })}</span></UtilityNotice>}
            {item.inspection?.hardLimitExceeded && <UtilityNotice className="mt-2" tone="error" role="alert"><AlertCircle className="mt-0.5 shrink-0" size={16} /><span>{t("features:excelCleaner.files.tooLarge", { count: item.inspection.cellCount.toLocaleString() })}</span></UtilityNotice>}
            {item.inspection && <div className="mt-3 grid gap-2" data-testid="excel-cleaner-sheets">{item.inspection.sheets.map((sheet) => { const selected = item.selectedSheets.includes(sheet.name); return <div className="grid grid-cols-[minmax(0,1fr)_auto] items-center gap-3 rounded-xl border border-border bg-muted/50 p-2.5 max-[620px]:grid-cols-1" key={sheet.name}><label className="flex min-w-0 items-center gap-2 text-sm"><input className="size-4 [accent-color:var(--color-green-700)]" type="checkbox" checked={selected} onChange={(event) => updateSelection(item.id, sheet.name, event.target.checked)} /><span className="min-w-0"><strong className="block overflow-hidden text-ellipsis whitespace-nowrap">{sheet.name}</strong><small className="text-xs text-muted-foreground">{sheet.rowCount.toLocaleString()} × {sheet.columnCount.toLocaleString()}</small></span></label>{selected && <UtilityField className="grid grid-cols-[auto_72px] items-center gap-2"><span>{t("features:excelCleaner.files.headerRow")}</span><UtilityInput className="h-8 rounded-lg px-2" type="number" min={1} max={Math.max(1, sheet.rowCount)} value={item.headerRows[sheet.name] ?? 1} onChange={(event) => updateHeader(item.id, sheet.name, Number(event.target.value))} onBlur={() => void refreshHeader(item.id, sheet.name)} /></UtilityField>}</div>; })}</div>}
            {item.inspection && <p className="mt-2 text-xs text-muted-foreground">{t("features:excelCleaner.files.unselectedOmitted")}</p>}
          </Card>)}
        </div>
      </UtilitySectionCard>

      <UtilitySectionCard step={2} title={t("features:excelCleaner.pipeline.title")} description={t("features:excelCleaner.pipeline.description")}>
        <div className="grid grid-cols-[minmax(0,1fr)_auto] items-end gap-2 max-[620px]:grid-cols-1" data-testid="excel-cleaner-add-rule"><UtilityField><span>{t("features:excelCleaner.pipeline.ruleType")}</span><UtilitySelect value={addType} onChange={(event) => setAddType(event.target.value as ExcelCleanerRuleType)}>{EXCEL_CLEANER_RULE_TYPES.map((type) => <option value={type} key={type}>{t(`features:excelCleaner.rules.${type}` as never)}</option>)}</UtilitySelect></UtilityField><Button type="button" variant="secondary" className="h-10 rounded-xl font-bold" onClick={addRule}><Plus size={16} />{t("features:excelCleaner.pipeline.add")}</Button></div>
        <div className="mt-3 grid gap-2.5" data-testid="excel-cleaner-rules" aria-label={t("features:excelCleaner.pipeline.orderLabel")}>
          {drafts.map((draft, index) => <Card as="article" key={draft.key} className="gap-0 overflow-visible rounded-2xl border border-border p-3 shadow-sm" data-testid="excel-cleaner-rule" draggable onDragStart={() => { dragRuleIndex.current = index; }} onDragOver={(event) => event.preventDefault()} onDrop={() => { if (dragRuleIndex.current !== undefined) moveRule(dragRuleIndex.current, index); dragRuleIndex.current = undefined; }}>
            <header className="mb-2 grid grid-cols-[auto_auto_minmax(0,1fr)_auto_auto_auto] items-center gap-1.5 max-[620px]:grid-cols-[auto_auto_minmax(0,1fr)]"><GripVertical className="text-muted-foreground" size={17} /><strong className="text-sm">{t("features:excelCleaner.pipeline.rule", { number: index + 1 })}</strong><span className="overflow-hidden text-ellipsis whitespace-nowrap text-xs text-muted-foreground">{draftTypeLabel(draft.text, translate)}</span><Button variant="ghost" size="icon-xs" type="button" onClick={() => moveRule(index, index - 1)} disabled={index === 0} aria-label={t("features:excelCleaner.pipeline.moveUp")}><ArrowUp size={15} /></Button><Button variant="ghost" size="icon-xs" type="button" onClick={() => moveRule(index, index + 1)} disabled={index === drafts.length - 1} aria-label={t("features:excelCleaner.pipeline.moveDown")}><ArrowDown size={15} /></Button><Button variant="ghost" size="icon-xs" className="text-destructive hover:bg-destructive/10 hover:text-destructive" type="button" onClick={() => removeRule(index)} aria-label={t("features:excelCleaner.pipeline.remove")}><Trash2 size={15} /></Button></header>
            <UtilityTextarea className="min-h-48 resize-y rounded-xl font-mono text-xs" data-testid="excel-cleaner-rule-settings" value={draft.text} onChange={(event) => updateDraft(index, event.target.value)} spellCheck={false} aria-label={t("features:excelCleaner.pipeline.settings", { number: index + 1 })} />
            {parsedPipeline.errors[index] && <p className="mt-1.5 text-sm font-bold text-destructive">{t("features:excelCleaner.pipeline.invalidRule")}</p>}
          </Card>)}
          {!drafts.length && <p className="rounded-xl border border-dashed border-border p-4 text-center text-sm text-muted-foreground">{t("features:excelCleaner.pipeline.empty")}</p>}
        </div>
        <details className="mt-3 rounded-2xl border border-border bg-muted/40 p-3" data-testid="excel-cleaner-json"><summary className="cursor-pointer text-sm font-bold">{t("features:excelCleaner.pipeline.json")}</summary><div className="mt-3 grid grid-cols-2 gap-3 max-[720px]:grid-cols-1"><UtilityField><span>{t("features:excelCleaner.pipeline.export")}</span><UtilityTextarea className="min-h-44 rounded-xl font-mono text-xs" readOnly value={exported} /></UtilityField><UtilityField><span>{t("features:excelCleaner.pipeline.import")}</span><UtilityTextarea className="min-h-44 rounded-xl font-mono text-xs" value={importText} onChange={(event) => setImportText(event.target.value)} /><Button className="mt-1 self-start rounded-xl" variant="secondary" type="button" onClick={applyImport}>{t("features:excelCleaner.pipeline.apply")}</Button></UtilityField></div></details>
        {(pipelineMessage || parsedPipeline.rootError) && <p className="mt-2 text-sm font-bold text-destructive" role="alert">{pipelineMessage ?? t("features:excelCleaner.pipeline.invalid")}</p>}
      </UtilitySectionCard>

      <UtilitySectionCard step={3} title={t("features:excelCleaner.output.title")} description={t("features:excelCleaner.output.description")}>
        ...
        <div className="mt-2 divide-y divide-border [&_[data-ui-component=toggle-row]]:min-h-[45px]">
        ...
        <UtilityNotice className="mt-3"><AlertCircle className="mt-0.5 shrink-0" size={16} /><span>{t("features:excelCleaner.output.formulaNote")}</span></UtilityNotice>
      </UtilitySectionCard>

      <Card className="mt-4 flex-row items-center justify-end gap-2 overflow-visible rounded-3xl border border-border p-4 shadow-md max-[620px]:grid max-[620px]:grid-cols-2 [&>*]:min-h-10" data-testid="excel-cleaner-actions"><Button type="button" variant="secondary" className="rounded-xl font-bold" onClick={() => void runPreview()} disabled={!firstReady || !parsedPipeline.pipeline || busy}><RefreshCw size={17} />{t("features:excelCleaner.actions.preview")}</Button><Button type="button" size="lg" data-ui-component="primary-button" className="h-12 rounded-2xl bg-green-700 text-[15px] font-bold text-white shadow-md shadow-green-700/20 hover:bg-green-800 focus-visible:border-green-700 focus-visible:ring-green-700/30" onClick={() => void run()} disabled={!readyItems.length || !parsedPipeline.pipeline || busy} aria-busy={busy}>{busy && <LoaderCircle className="animate-spin" size={19} aria-hidden="true" />}<FileSpreadsheet size={18} />{t("features:excelCleaner.actions.run", { count: readyItems.length })}</Button></Card>
      {busy && <div className="mt-2 flex justify-end"><Button className="rounded-xl" variant="destructive" type="button" data-testid="excel-cleaner-cancel" onClick={() => controllerRef.current?.abort()}><X size={16} />{t("features:excelCleaner.actions.cancel")}</Button></div>}
      <OperationProgress {...operation} accent="green" title={t("features:excelCleaner.progress.title")} />

      {(preview || previewError) && <Card as="section" className="mt-4 gap-0 rounded-3xl border border-border p-4 shadow-sm" data-testid="excel-cleaner-preview"><div><p className="text-xs font-extrabold tracking-[.08em] text-green-700 uppercase dark:text-green-300">PREVIEW</p><h2 className="mt-1 font-heading text-xl font-medium">{t("features:excelCleaner.preview.title")}{previewStale && <span className="ml-2 rounded-full bg-amber-500/10 px-2 py-1 align-middle text-xs font-bold text-amber-800 dark:text-amber-300" data-testid="excel-cleaner-preview-stale">{t("features:excelCleaner.preview.stale")}</span>}</h2><p className="mt-2 text-sm text-muted-foreground">{preview ? t("features:excelCleaner.preview.summary", { rows: preview.summary.outputRows, changed: preview.summary.changedCells }) : previewError}</p></div>{preview?.stages.at(-1)?.sample.length ? <div className="mt-3 overflow-x-auto rounded-xl border border-border"><table className="w-full border-collapse text-sm [&_td]:border-t [&_td]:border-border [&_td]:px-3 [&_td]:py-2 [&_th]:bg-muted [&_th]:px-3 [&_th]:py-2 [&_th]:text-left"><tbody>{preview.stages.at(-1)!.sample.map((row, rowIndex) => <tr key={rowIndex}>{row.map((value, columnIndex) => rowIndex === 0 ? <th key={columnIndex}>{value}</th> : <td key={columnIndex}>{value}</td>)}</tr>)}</tbody></table></div> : null}</Card>}

      {(downloads.length || failures.length) && <Card as="section" className="mt-4 gap-0 overflow-visible rounded-3xl border border-border p-4 shadow-sm" data-testid="excel-cleaner-results"><div><p className="text-xs font-extrabold tracking-[.08em] text-green-700 uppercase dark:text-green-300">RESULTS</p><h2 className="mt-1 font-heading text-xl font-medium">{t("features:excelCleaner.results.title")}</h2><p className="mt-2 text-sm text-muted-foreground">{t("features:excelCleaner.results.description", { success: downloads.length, failed: failures.length })}</p></div>
        {csvRiskCount > 0 && !csvSafeMode && <UtilityNotice className="mt-3" data-testid="csv-risk-warning"><AlertCircle className="mt-0.5 shrink-0" size={16} /><span>{t("features:excelCleaner.results.csvRisk", { count: csvRiskCount })}</span></UtilityNotice>}
        <div className="mt-3 grid grid-cols-2 gap-2 max-[620px]:grid-cols-1">{downloads.map((item) => <Button render={<a href={item.url} download={item.fileName} data-testid="excel-cleaner-download" />} variant="secondary" className="h-auto min-h-12 justify-start rounded-xl px-3 py-2 text-left" key={`${item.fileId}-${item.fileName}`}><Download size={18} /><span className="min-w-0"><strong className="block">{item.kind.toUpperCase()}</strong><small className="block overflow-hidden text-ellipsis text-xs text-muted-foreground">{item.fileName} · {formatBytes(item.blob.size)}</small></span></Button>)}{zip && <Button render={<a href={zip.url} download={zip.fileName} data-testid="excel-cleaner-download" />} variant="secondary" className="h-auto min-h-12 justify-start rounded-xl px-3 py-2 text-left"><Download size={18} /><span className="min-w-0"><strong className="block">{t("features:excelCleaner.results.zip")}</strong><small className="block overflow-hidden text-ellipsis text-xs text-muted-foreground">{zip.fileName} · {formatBytes(zip.size)}</small></span></Button>}</div>
        {failures.map((item) => <UtilityNotice className="mt-2" tone="error" role="alert" key={`${item.fileName}-${item.message}`}><AlertCircle className="mt-0.5 shrink-0" size={16} /><span className="flex flex-col"><strong>{item.fileName}</strong>{item.message}</span></UtilityNotice>)}
      </Card>}

      <ToolGuide title={t("features:excelCleaner.guide.title")} description={t("features:excelCleaner.guide.description")} blocks={t("features:excelCleaner.guide.blocks", { returnObjects: true }) as any} faq={(t("features:excelCleaner.guide.faq", { returnObjects: true }) as unknown as Array<{ q: string; a: string }>).map((item) => ({ question: item.q, answer: item.a }))} />
    </div>
  </UtilityPage>;
