  ...
  return <UtilityPage toolId="excel-cleaner">
    <div className="contents" data-testid="excel-cleaner-page">
      ...
      <UtilitySectionCard step={1} title={t("features:excelCleaner.files.title")} description={t("features:excelCleaner.files.description")}>
        ...
        <div className="mt-3 grid gap-2.5" data-testid="excel-cleaner-files">
          {items.map((item, index) => <Card as="article" className="gap-0 overflow-visible rounded-2xl border border-border p-3 shadow-sm" key={item.id} data-testid="excel-cleaner-file" data-inspecting={item.inspecting || undefined}>
            ...
            {item.error && <UtilityNotice className="mt-2" kind="error" role="alert"><span>{item.error}</span></UtilityNotice>}
            {item.inspection?.softLimitExceeded && !item.inspection.hardLimitExceeded && <UtilityNotice className="mt-2"><span>{t("features:excelCleaner.files.large", { count: item.inspection.cellCount.toLocaleString() })}</span></UtilityNotice>}
            {item.inspection?.hardLimitExceeded && <UtilityNotice className="mt-2" kind="error" role="alert"><span>{t("features:excelCleaner.files.tooLarge", { count: item.inspection.cellCount.toLocaleString() })}</span></UtilityNotice>}
            {item.inspection && <div className="mt-3 grid gap-2" data-testid="excel-cleaner-sheets">{item.inspection.sheets.map((sheet) => { const selected = item.selectedSheets.includes(sheet.name); return <div className="grid grid-cols-[minmax(0,1fr)_auto] items-center gap-3 rounded-xl border border-border bg-muted/50 p-2.5 max-[620px]:grid-cols-1" key={sheet.name}><label className="flex min-w-0 items-center gap-2 text-sm"><input className="size-4 [accent-color:var(--primary)]" type="checkbox" checked={selected} onChange={(event) => updateSelection(item.id, sheet.name, event.target.checked)} /><span className="min-w-0"><strong className="block overflow-hidden text-ellipsis whitespace-nowrap">{sheet.name}</strong><small className="text-xs text-muted-foreground">{sheet.rowCount.toLocaleString()} × {sheet.columnCount.toLocaleString()}</small></span></label>{selected && <UtilityField className="grid grid-cols-[auto_72px] items-center gap-2"><span>{t("features:excelCleaner.files.headerRow")}</span><UtilityInput className="h-8 rounded-lg px-2" type="number" min={1} max={Math.max(1, sheet.rowCount)} value={item.headerRows[sheet.name] ?? 1} onChange={(event) => updateHeader(item.id, sheet.name, Number(event.target.value))} onBlur={() => void refreshHeader(item.id, sheet.name)} /></UtilityField>}</div>; })}</div>}
      ...
      <UtilitySectionCard step={3} title={t("features:excelCleaner.output.title")} description={t("features:excelCleaner.output.description")}>
        ...
        <UtilityNotice className="mt-3"><span>{t("features:excelCleaner.output.formulaNote")}</span></UtilityNotice>
      </UtilitySectionCard>
      ...
      {(downloads.length || failures.length) && <Card as="section" className="mt-4 gap-0 overflow-visible rounded-3xl border border-border p-4 shadow-sm" data-testid="excel-cleaner-results"><div><p className="text-xs font-extrabold tracking-[.08em] text-primary uppercase dark:text-primary">RESULTS</p><h2 className="mt-1 font-heading text-xl font-medium">{t("features:excelCleaner.results.title")}</h2><p className="mt-2 text-sm text-muted-foreground">{t("features:excelCleaner.results.description", { success: downloads.length, failed: failures.length })}</p></div>
        {csvRiskCount > 0 && !csvSafeMode && <UtilityNotice className="mt-3" data-testid="csv-risk-warning"><span>{t("features:excelCleaner.results.csvRisk", { count: csvRiskCount })}</span></UtilityNotice>}
