...
- **실수집 확인 — 역사적 백필**: 엄격한 기관·보고서 필터로 첫 공식 접수 범위를 확정한 뒤 `foliotrace-data`에서 수동 시작하고, 별도 커서를 통해 매일 재개한다. 오래된 접수에서 원문 파싱 실패가 발생하면 목록 확인과 수량 미확인을 구분해 남기고 원인을 조사한다. 운영 완료 창·신규 접수·원문 재확인 수는 첫 실제 Actions 실행 결과로 기록한다. — Codx
