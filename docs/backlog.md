...
## FolioTrace 생산 차단·후속 (2026-09-26)

- **차단 — 네이버 시세 재이용 권한**: Npay 증권 공식 도움말은 시세 재가공·웹페이지 사용과 재배포를 금지한다. 사용권을 입증한 계약 또는 사용자가 승인한 합법적인 대체 공급자와 검증된 KRX 정규장 종가가 있어야 가격 평가·첫 생산 게시가 가능하다. 현 코드의 `price-and-value`는 실패하도록 막혀 있다. — Codx
- **차단 — DART 첫 live catch-up과 원격 state**: 로컬에 `DART_API_KEY`가 없고 `foliotrace-data` seed commit `413c021`은 원격 미반영이다. 승인된 비밀 주입·전용 브랜치 원격 반영 뒤 2026-09-08 관측 힌트 주변부터 실제 목록 완전성을 수립해야 한다. 과거 전체 coverage는 여전히 미검증이다. — Codx
- **후속 — 공시 정정·취소/metadata 복구**: 원문 파서·구조화 API 정확 접수번호 대조·`rm` 상태·durable unresolved 큐는 있으나 정정 원접수번호의 확인된 연결과 철회 확정, metadata 미해결 596건의 한정 복구, 실제 시세 입력을 통한 게시 스냅샷은 완료되지 않았다. C08·B07·D01~D12·E03/E09~E11의 생산 판정은 보류한다. — Codx
