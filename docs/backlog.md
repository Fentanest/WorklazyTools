...
- **후속 — 거래일·분할 검증**: 네이버 분봉은 날짜·종목이 맞는 KRX 확정 종가와 매 종목 대조한다. 미확인 휴장·특수 개장일에 기준일이나 확정 가격이 맞지 않으면 게시를 멈춘다. 상장주식 수가 접수일 이후 달라졌고 분할·병합 또는 권리 반영 근거가 없으면 해당 보유를 미평가로 남긴다. 공식 달력의 일반 연도 조회와 사건별 직접 연결은 후속으로 남긴다. — Codx
