...
- **차단 — 첫 live 검증**: GitHub Secret에 `DART_API_KEY`가 있으나 로컬에는 값이 없고, `foliotrace-data` seed HEAD `11d4175`는 원격 미반영이다. 첫 CI catch-up, 보유 증권종류 대조, 네이버 종가 및 정적 게시 스냅샷은 실제 실행 후 수치·결함을 판정한다. 과거 전체 공시 coverage는 여전히 미검증이다. — Codx
- **후속 — 증권종류·공시 정정·metadata 복구**: 원문 수량의 '주식등'과 의결권 주식량 일치만으로 보통주를 단정하지 않는다. 별도 종목코드·증권종류 근거가 없는 보유는 평가에서 제외한다. 정정 원접수번호의 확인된 연결·철회 확정, metadata 미해결 596건의 한정 복구, 실제 다일 추이는 CI 실증 전이다. — Codx
