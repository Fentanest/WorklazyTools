...
- **BNK 원문 ZIP 실증과 과거 범위 확대**: 공개 DART viewer의 2025-12-31 BNK 주주·주식총수 표는 보통주 단일 종류를 뒷받침한다. 같은 접수의 OpenDART ZIP을 Secret-backed 검증 전용 실행에서 다시 파싱해 source_claims·정확 종목·캐시 재처리 결과를 확인해야 한다. 이 공시는 현재 보유 기준일이 아니라 2025년 말 과거 관측이다. 다른 공시 구조에 이 규칙을 무차별 적용하지 않는다. — Codx
- **간접 자료 재검토와 범위 확인**: KT&G 최신 확인 수치 2025-08-22 8.16% 뒤 2025-12-31 수량 없는 감소가 있어 현재 정확한 비율은 미확인이다. 직접 공시 7.5%와 증권 종류·지분율 계산 범위가 같다는 근거도 추가 확인이 필요하다. 대상 접수의 원문 ZIP 캐시는 재사용하지만 기존 본문검색 후보의 캐시는 파싱 결과와 해시만 저장해 파서 변경 시 원문을 다시 조회한다. 전체 유형 목록·본문검색의 오래된 구간은 계속 재개해야 한다. — Codx
- **제3자 공시 본문검색 및 원문 해석**: DART 본문색인·지분/의결권 유형의 1999~2005·2006~2008·2006~현재 독립 커서를 자동 재개한다. 원문 확인 표본 9건은 검색 완료 전체 결과가 아니며, 후보 중 `source_context_review_pending`은 종목·수량·기준일을 사람/별도 규칙으로 검증하기 전 보유 원장에 넣지 않는다. 검색 결과는 문서당 하나의 발췌문이라 주주 문맥을 놓칠 수 있다. 완료 창의 페이지 digest·기각 수·기각 ID 표본을 보존하고, 원문 근접 문맥 후보도 종목·수량 자동 확정 없이 재검토 대기로 둔다. 검색 색인 수량과 원문 마스킹이 충돌한 2006 삼성전자·포스코 권유 문서는 수량을 미확인으로 유지한다. 2000 KB캐피탈 배정 결의의 1,533,800주는 예정 수량이며 실제 보유로 승격하지 않는다. 2005-01-25는 2006 포스코 공시가 언급한 직전 보고서 날짜이며 원문 접수 재확인은 미완료다. — Codx
- **본문검색 음성 휴리스틱 재검토**: 발췌문에 주식 단어가 없는 문서도 압축 receipt/document 큐에 전부 보존해 제한된 원문 조회 용량으로 재검토한다. `source_mention_no_equity_context`는 검사한 원문에서 가까운 주식 문맥을 못 찾았다는 휴리스틱 결과일 뿐 실제 보유 부재의 증명이 아니다. 원문 문맥 양성도 회사·기준일·수량 자동 추출 없이 해석 대기로 남는다. — Codx
