...
- **배포 CI 환경 교정**: 첫 push `29ba546`의 self-hosted Pages runner에는 시스템 `ffmpeg`/`ffprobe`가 없어 신규 스모크가 fixture 생성 전 `spawn ffmpeg ENOENT`로 종료됐다(제품·테스트 로직 실패 아님). smoke 직전에 `AnimMouse/setup-ffmpeg` v1.2.5 commit `178e0b4a408f06ce40d896c3cd79f50fa6f8b0c3`과 FFmpeg 7.1을 고정 설치하도록 보완하고 재실행 결과를 배포 run으로 확인했다.
