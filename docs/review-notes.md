...
- **S9 광고 제외 경로 문서 교체 실패 & 구현 제외:** S9는 보호 가드가 광고 제외 경로(`/ko/tools/pdf-editor` 등)로의 이동을 가로채고 적절한 전환을 실행해야 한다는 계약이었다. 그러나 실측 결과 **광고 제외 경로로 이동했을 때 전체 문서 교체가 일어나지 않아 이전 화면의 광고 script가 남는다**(scripts = 1, 문서 교체 loaderId 변경 0). 원인 후보로 캡처 단계 클릭 리스너의 `pendingTargetRef` 미설정을 고쳤으나 해결되지 않았고 근본 원인 미규명. 광고 정책 위반 위험을 새로 만들지 않기 위해 **이번 배포에서 S9 보호 가드는 제외**한다. — Claude
