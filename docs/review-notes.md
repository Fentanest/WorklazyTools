...
- **unit 실패 10→4 귀속 갱신:** 통합 후보에서 전체 605건을 끝까지 실행해 601건 통과·4건 실패를 확인했다. 확장자 없는 import 3건은 `.ts` 확장자를 명시해 실제 하위 테스트가 실행되도록 수리했고, SEO 2건은 승인된 카피와 현 FAQ 개수에 맞췄으며, `feature-locales` 1건은 `guides.json`의 실제 선택 내용 검사로 이관했다. 기대를 낮추지 않았다. 해소 6건 외 잔여는 이름이 같은 `p1b-components` 3건과 `ui-legacy-isolation` 1건이며 `adsense-followup2` 계획으로 이월한다. — Muse/Codx
