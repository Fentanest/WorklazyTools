...
- 첫 운영 backfill Actions 36253215622는 첫 완료 창 전에 RuntimeError로 종료해 data branch 역사 커서가 생성되지 않았다. 원인은 이 로그만으로 특정할 수 없으므로 원문 예외·키 포함 URL을 출력하는 방식을 기각했다. 첫/후속 페이지 요청, 페이지 검증, NPS 행 적용의 실패 코드를 날짜 창·페이지 번호 같은 안전한 값과 함께 기록하고, probe에서 검증된 날짜 오름차순 요청을 본 수집에도 적용했다. 재실행의 실패 코드와 실제 수정 여부는 별도 판정한다. — Codx
