...
## 2026-09-27 — FolioTrace 역사적 목록 백필 경계

- 통합 후보 `d74f67d`의 Astra 영향 검수는 차단 0건 PASS였다. 기준일 충돌 × 직접 공시 분모 확인 여부 4변형과 정상 회사 단위 반영 2변형을 별도 재현했고 Python 179건 로그 해시와 맞췄다(원문: 비추적 `docs/jobs/todo/foliotrace/review-evidence/astra-d74f67d/review-result.json`). Claude 최종 영향 감사도 차단 0건 PASS로 기록됐다(원문: 비추적 `docs/jobs/todo/foliotrace/claude-b3-audit-d74.json`). 이는 코드 검수 결과이며 원격 수집·게시 완료 여부는 별도 확인한다. — Codx
- 직접 D001 원문의 `THS_*` 비율은 의결권 있는 주식·보고자와 특별관계자 합산 범위이고, BNK형 사업보고서의 주주명부 행은 국민연금 단독·발행주식수 기준이다. 이 둘을 같은 분모·보유주체로 간주해 `4.80→5.05`의 `+0.25%p`나 직접 공시 재진입을 만드는 안은 기각했다. 다만 제3자 행의 대상 회사·보통주만 발행된 사실·같은 날짜의 발행주식수·국민연금 명의·명시 기준일·정확 지분율이 확인되면 `5.05%` 자체는 그 범위를 붙여 최신 확인값으로 보인다. 수량이 원문에서 가려졌으면 수량과 평가금액은 비워 둔다. 접수일이 늦어도 직접 공시의 확인된 보유 기준일이 더 과거면 제3자 수치를 차단하지 않는다. 생산 표기와 파서 출력을 연결한 `6.80→7.80→4.80→5.05` 회귀로 이 경계를 확인했다. — Codx
- 동일 기준일의 제3자 확인값 `5.05`와 `5.16`이 충돌하면 기존 직접 공시의 분모 정보가 아직 없어도 최신 표의 지분율·평가를 보류한다. 이미 더 새 기준일의 직접 공시가 확인됐다면 과거 충돌은 관측 이력에서만 드러내고 그 최신값은 유지한다. — Codx
- 제3자 확인값 충돌 상태에서 더 약한 회사 단위 관측을 현재값으로 덮는 안은 기각했다. 실제 충돌은 해결되지 않았고, 덮으면 표의 충돌 설명과 지분율이 모순되고 기존 직접 공시 baseline도 잃는다. 회사 단위 관측은 별도 이력에 남겨 출처 확인을 계속할 수 있다. — Codx
- [DART BNK금융지주 사업보고서 2026-03-18](https://dart.fss.or.kr/dsaf001/main.do?rcpNo=20260318001147)의 공개 viewer dcmNo11140481에서 주식총수 절(eleId3, offset16085, length155052; SHA-256 `2e42ea42b68201cd3e81f5a3d53f1d76f8ed6e24510f98f151eafb66124f36d9`)은 2025-12-31 발행 보통주 310,327,033·종류주식 0·합계 310,327,033을 보인다. 주주 절(eleId48, offset4926538, length72123; SHA-256 `43b1e295e1f96e209ca73ef0451b882c0fd44848f4504ba9adba9130c95ea780`)은 같은 날짜 NPS 26,755,051주/8.62%, 같은 월말 주주명부·발행주식총수 310,327,033 주석을 보인다. 두 공개 절을 결합한 읽기 전용 parser probe에서 보통주 관측 1건이 생성됐으며 Decimal 표시 반올림도 일치한다. 검증 전용 Actions `36270038754`에서 공식 목록 1건·OpenDART ZIP 1건을 실제 조회해 동일 관측 1건을 재현했다. ZIP SHA-256 `087b8f4625a4cac3948e20227655aacf37d66c007a390e18eb6b009ecd841ffb`, XML SHA-256 `1c63bf21fe8090a1c734a5869aa3fa4e1520c6d7ae8285a94cf5cc000bdbcfcc`, 주주 행 SHA-256 `533ca9cacf4451ad7d42febabb41c05b70800d18ab3a81bd3e25486b1b367625`이다. 이 실행은 데이터를 저장하지 않았고, 더 최신 직접 공시 7.57%는 그대로 유지했다. — Codx
- 원문 대상 재검사 Actions `36268226921`·`36268296633`·`36268506724`·`36268569761`은 게시 없이 실행됐다. KT&G 2025년 3분기·2026년 1분기 공시에서 국민연금 지분 2023-06-30 6.31%/8,662,320주와 2025-08-22 8.16%/9,954,722주를 확인했다. 후자는 두 공시에 같은 기준일로 재인용되어 한 관측으로 묶었다. 2025-12-31에는 국민연금 소유 감소가 적혔지만 새로운 수량·비율이 없어 마지막 수치 8.16%의 기준일을 유지하고 현재 추적 상태는 미확인으로 둔다. 직접 공시 7.5%와 범위가 확인되지 않아 증감률·평가금액은 계산하지 않는다. 대상 경로는 저장한 ZIP의 파서 재실행에서 원문 재요청 0건·캐시 적중 1건·신규 사실 0건을 실측했다. — Codx
- 위 실행에서 사용자 제공 포스코 권유 접수 `20060208000020`은 수량이 `*****`인 별도 간접 단서로 남았다. 실제 수량 2,407,509주/2.76%의 과거 관측은 다른 접수 `20060124800040`에 있으며 둘을 동일시하지 않는다. KB 접수 `20081007000308`은 공식 목록 확인 후 현재 종목 원장과 대상 회사가 달라 매핑 대기로 보존했고, 현재 보유로 올리지 않았다. 두 정확한 접수의 대상 재검사에서 구조화 수치 주장은 0건이었다. — Codx
- [DART 전자공시 개요](https://dart.fss.or.kr/introduction/content1.do)는 1999년 4월 일부 전자공시 시작, 2000년 3월 전체 공시 확대를 설명한다. 이전 범위 본문검색은 1999-04-01부터 진행하며, 그 앞 구간의 NPS 보유 부재를 뜻하지 않는다. 검색 사이트는 문서당 한 발췌문만 보이므로 주식 단어가 없는 hit를 완전히 버리면 다른 섹션의 지분 사실을 놓칠 수 있다. 완료 창마다 모든 비후보 receipt/document ID를 압축 큐와 count/hash로 보존하고, 후보 우선 처리 후 남은 원문 조회 용량으로 재검토한다. 조회 실패 XML은 ZIP 원문으로 오인해 영구 캐시하지 않는다. — Codx
- [DART KB캐피탈 2000-04-20](https://dart.fss.or.kr/dsaf001/main.do?rcpNo=20000420000129&dcmNo=51683)의 제3자 배정 섹션은 국민연금관리공단에 보통주 1,533,800주를 배정할 **계획**을 적는다(원문 섹션 13,105 bytes, SHA-256 `6ea79c3d8f89f763caa3c9c45357c051977a4c3a22907b3067aae6cf30d54e26`). 실제 청약·납입·보유 완료를 이 결의만으로 확인할 수 없으므로 예정 수량을 현재·과거 보유 원장에 넣지 않는다. [DART SBS 2005-01-31](https://dart.fss.or.kr/dsaf001/main.do?rcpNo=20050131000208&dcmNo=980717)의 의결권 권유 피권유자 명단은 국민연금관리공단을 보통주 소유자로 적지만 수량은 가린다(원문 섹션 10,997 bytes, SHA-256 `cc8d8a7da1e4e487338c6bd1d58a97ab878f6b5cd2fc243db9e071281ade11f5`). 둘 다 국민연금이 제출한 D001 대량보유 신고가 아니다. — Codx
- [DART 공식 공시통합검색 안내](https://dart.fss.or.kr/guide/contents122.jsp)는 본문내용 검색을 제공하며, OpenDART `list.json`은 본문 키워드 인자가 없다. 공개 `/dsab007/search.ax`의 전체 본문 범위(문서유형 제한 없음)에서 2006-02-08 `국민연금공단` 검색은 포스코 접수 `20060208000020` 1건, 2008-10-07 같은 검색은 KB금융 관련 `20081007000289`와 `20081007000308` 2건을 반환했다. 서버는 `maxResults=100`을 보내도 실제 10행/페이지이며, 2006년 `국민연금관리공단`은 14,119건/1,412페이지라 날짜창을 100페이지 이하로 분할해야 한다. 2006-02-01~07 실제 전체검색은 세 이름 총 79 hit/78 고유 문서/10페이지, 주식 문맥 후보 6·그 외 72로 기록됐다. 주소·퇴직급여 잡음 때문에 검색 적중만으로 보유를 인정하지 않는다. 완료 페이지의 digest·건수와 후보/원문 재검토 상태는 별도 원장으로 저장하고, 날짜창 전체가 성공해야 커서를 전진시킨다. — Codx
- [DART 2003-09-09 KT 보고서](https://dart.fss.or.kr/dsaf001/main.do?rcpNo=20030909000232&dcmNo=674676)의 [보고자 섹션](https://dart.fss.or.kr/report/viewer.do?rcpNo=20030909000232&dcmNo=674676&eleId=57&offset=4090&length=6213&dtd=dart2.dtd)(3,765 bytes, SHA-256 `9bc0ffeb87e2546913ce281a92f54952786f61e9159f4ee7ca0c5a9cfb36196d`)은 국민연금관리공단의 **KT 지분율 2.70%**를 적되 2002년 12월 말 주주명부 기준이며 현재 수량이 다를 수 있다고 명시한다. 2003 공시일을 2002 기준의 현 보유일로 쓰지 않는다. — Codx
- [DART 2006-02-01 포스코 최대주주 변동](https://dart.fss.or.kr/dsaf001/main.do?rcpNo=20060124800040&dcmNo=1248144)의 [HTML 원문](https://dart.fss.or.kr/report/viewer.do?rcpNo=20060124800040&dcmNo=1248144&eleId=0&offset=0&length=0&dtd=HTML)(EUC-KR 26,691 bytes, SHA-256 `1ec063f0998a40c96f9a420643a9898025a64a042a1572aa5f1c8c199ce86306`)은 국민연금관리공단 보통주 장내매도 -676,677주와 직전 3,084,186주/3.54%→이번 2,407,509주/2.76%를 적는다. 원문이 인용한 직전 보고서 제출일 2005-01-25는 해당 보고서 자체를 독립 확인한 날짜가 아니므로 참조값으로만 남긴다. — Codx
- [삼성전자 2006-02-01 권유 문서](https://dart.fss.or.kr/dsaf001/main.do?rcpNo=20060201000258&dcmNo=1247987)의 피권유자 섹션(641,725 bytes, SHA-256 `591344ab07a63e89b0e8c8b68e4db8a9e87639cf49f385b68bbc4453860ed32c`)과 [포스코 2006-02-08 권유 문서](https://dart.fss.or.kr/dsaf001/main.do?rcpNo=20060208000020&dcmNo=1251611)의 같은 섹션은 국민연금을 보통주 소유자로 적으나 주식수를 `*****`로 숨긴다. 검색 색인의 숫자는 원문과 충돌하므로 수량 검증값으로 승격하지 않는다. [KT 2006-04-07 보고자 섹션](https://dart.fss.or.kr/report/viewer.do?rcpNo=20060407000047&dcmNo=1336865&eleId=56&offset=4039&length=38384&dtd=dart2.dtd)(25,108 bytes, SHA-256 `3d25860940934c48ff3d7fda5c77bcedde9be50aee2e3cc734330f055fd09cc7`)의 3.4%는 KT 지분으로, 문서 대상인 케이티프리텔 지분이 아니다. — Codx
- [KB금융 2008-10-07 주식교환 종료보고서](https://dart.fss.or.kr/dsaf001/main.do?rcpNo=20081007000289&dcmNo=2031973)의 [주주 변동 섹션](https://dart.fss.or.kr/report/viewer.do?rcpNo=20081007000289&dcmNo=2031973&eleId=104&offset=7282&length=44531&dtd=dart2.dtd)(23,580 bytes, SHA-256 `d8ed336da5d21eb9321ffc80b69140367891e6f504b1b503a5ee7ebc87f9985c`)은 교환 전 국민은행 17,910,781 보통주/5.32%(2008-09-26 기준)와 교환 후 KB금융 17,910,781 보통주/5.03%를 별도로 적는다. 회사·단계가 달라 하나의 보유 행이나 당일 가격 평가로 합치지 않는다. — Codx
- [DART 접수 20060208000020](https://dart.fss.or.kr/dsaf001/main.do?rcpNo=20060208000020&dcmNo=1251611)은 `POSCO홀딩스/의결권대리행사권유참고서류/2006.02.08` 문서이며 표지의 권유자는 주식회사 포스코다. [피권유자 명단 원문](https://dart.fss.or.kr/report/viewer.do?rcpNo=20060208000020&dcmNo=1251611&eleId=265&offset=26246&length=39219&dtd=dart2.dtd)은 국민연금공단을 연번 2의 포스코 보통주 소유자로 적지만 주식수는 `*****`로 가린다(해당 섹션 18,765 bytes, SHA-256 `a1b36c74f0e45dfad024671240bf1731ad8bde0ea9175d8f2625196333eb4f0c`). 지분율도 없다. 이는 직접 국민연금 제출 D001 접수나 당시 정확한 수량을 입증하지 않으므로 별도 간접 근거로만 보존한다. — Codx
- [DART 접수 20081007000308](https://dart.fss.or.kr/dsaf001/main.do?rcpNo=20081007000308&dcmNo=2032024)는 `국민은행/주식등의대량보유상황보고서(일반)/2008.10.07` 문서다. 표지의 보고자는 KB금융지주이고, [제1부 2절 원문](https://dart.fss.or.kr/report/viewer.do?rcpNo=20081007000308&dcmNo=2032024&eleId=56&offset=4038&length=14632&dtd=dart2.dtd)은 국민연금공단을 **보고자 KB금융지주의 최대주주(5.03%)**로 서술한다(해당 섹션 응답 7,898 bytes, SHA-256 `228e9155a30c48d5b1f6bcebaa129b58acf662c4db311aafbe581b9f70f84458`). 따라서 이는 국민연금이 국민은행 보유 수량을 신고한 접수가 아니며 엄격한 제출인 필터의 제외가 맞다. 2008-10-07 접수 문서의 제3자 언급을 별도 근거로 보존하고, 직접 제출 접수·보유 수량·평가액에는 포함하지 않는다. — Codx
- 실제 역사 백필 Actions 36254181893은 목록 요청 349건과 원문 파싱 120건, 완료 창 6개를 3분 25초에 처리했다. 공식 OpenDART 가이드의 오류 020은 일반적으로 2만 건 이상에서 발생하지만 키별 제한은 다를 수 있다. 이 실측 범위에서 일일 schedule 상한을 listing 500페이지·신규 원문 300건으로 올리고, 데이터 브랜치 CAS·Pages concurrency·원문 재확인 10건 상한은 유지한다. — Codx
- 첫 운영 backfill Actions 36253215622는 첫 완료 창 전에 RuntimeError로 종료해 data branch 역사 커서가 생성되지 않았다. 원문 예외·키 포함 URL 대신 단계별 고정 오류 코드를 추가했고, 재실행 36253552322에서 첫 80일 창 page63 row25의 `NPS_DATE`를 확인했다. 실제 `rcept_dt`는 유효하지만 접수번호 앞 8자리와 다르다. 공식 가이드가 두 값의 동등성을 보장하지 않으므로 공식 목록의 `rcept_dt`를 접수일로 쓰고 접두 날짜 불일치는 별도 사실로 남긴다. 기존 이관 접수일과 공식 날짜의 충돌은 별도 미해결·평가 제외 경계를 유지한다. — Codx
- 접수번호 접두 날짜 불일치가 오늘자 증분에도 등장하면 접수번호의 사전식 순서로 현재 보유를 안전하게 교체할 수 없다. 과거 백필은 공식 목록일의 원문 이벤트를 보존하되, 일일 경로에서 같은 접수를 만나면 `receipt_chronology_unverified`로 남기고 현재 보유 승격·원문 재확인·공개 이벤트를 보류한다. 공개 보유의 최신 미해결 판정은 공식 목록 날짜와 접수번호의 순서쌍으로 비교해, 번호가 더 작지만 실제 접수일이 뒤인 공시도 평가 제외에 반영한다. — Codx
- [OpenDART 목록 가이드](https://opendart.fss.or.kr/guide/detail.do?apiGrpCd=DS001&apiId=2019001)의 회사 미지정 최대 3개월 조건에 맞춰 80일 창을 사용한다. 모든 페이지의 건수·번호·중복을 확인한 창만 별도 역사적 커서에 기록한다. 오래된 접수를 오늘자 보유에 승격하거나 과거 가격으로 평가 이력을 만들어내는 경로는 채택하지 않았다. 확인된 접수 원문 수량·지분율과 이관 자료의 원래 값은 별도 필드로 보존한다. — Codx
- 1999–2004년 D001 사전 탐색에서는 넓은 `국민연금` 조건의 일치가 0건이었다. 2005년 broad match 20건의 첫 10건은 실제 기관이 아닌 벤처조합이었다. 엄격한 기관 제출인·대량보유 보고서명으로 2005–2008년 0건, 첫 국민연금공단 D001 접수는 2009-02-27 `20090227000244`로 확인됐다(GitHub Actions 36251948468, 2009년까지 목록 요청 970건). 첫 세 접수는 구조화 행이 없었지만 DART 문서에서 수량·지분율을 파싱했다. 운영 백필 시작은 누락 위험을 피하도록 2009-01-01로 둔다. 이후 원문 파싱 실패는 검증되지 않은 접수로 남기고 다음 날짜에 한정 재시도한다. — Codx

## 2026-09-26 — FolioTrace 구현 중 판정

- [KRX 공식 연도별 휴장일](https://open.krx.co.kr/contents/MKD/01/0110/01100305/MKD01100305.jsp)의 2026 조회 응답은 17개 날짜이며 9월 24·25일과 일반 평일인 10월 9일을 포함한다. 수집·가격 경로가 이 연도별 응답을 같은 기준일 계산에 사용한다. [2025년 11월 13일 거래시간 변경 공시](https://kind.krx.co.kr/external/2025/10/30/000102/20251030000137/99303.htm)는 당일 정규장 10:00–16:30을 명시하므로 해당 날짜의 16:30 종가 경로만 허용한다. 추후 특수 거래일은 날짜별 공식 발표를 확인해 추가한다. — Codx

- MyTradingDesk의 실제 `data/context-service/nps`를 서비스 변경 없이 읽었다. 허용 파일 129개를 두 번 대조한 source hash는 `6048f7bfdd2a7d677f914ceadd0ce19318a584fa03746e3b00587f29ac22479d`; 정규화 import는 접수번호 1,489개, 중복 참조 401개 병합, metadata 미해결 596개, universe 561개, 보유 근거 285개다. 접수일 관측 범위는 2020-01-06~2026-09-08. `state.last_run_at`과 holdings `as_of_date`는 목록 완전성 증거가 아니므로 과거 coverage는 `unverified`, 재개 기준은 실제 관측 접수일 2026-09-08의 힌트로 둔다. 원격 전용 데이터 브랜치 `66d6250`에는 과거 이벤트 893건(기존 live 2건 포함)과 검증된 종가 캐시가 보존되며 원본 보강 2회차는 무변경이었다. — Codx
- 네이버 `basic`·일별 응답의 2026-09-23 종가는 000660=1,863,000, 005930=286,500, 0126Z0=331,000이나 각각 20시 이후 최종값이다. 네이버 `fchart.stock.naver.com/sise.nhn`의 15:30 분봉은 1,862,000 / 285,500 / 330,000으로 [KRX KIND 종가](https://kind.krx.co.kr/common/stockprices.do?isurCd=00066&method=searchStockPricesMain)와 세 종목 모두 일치한다. 추가 분산 10개 종목도 10/10 일치했다. 기존 basic/일별 캐시는 평가 가격으로 재사용하지 않고, 정확한 거래일의 네이버 분봉을 같은 날짜·정확한 종목/회사에 대한 KRX 확정 종가와 **매 종목** 대조한다. SK스퀘어는 15:30 행 없이 15:32 첫 체결값 1,190,000이 [KRX KIND 공식 종가](https://kind.krx.co.kr/common/stockprices.do?isurCd=40234&method=searchStockPricesMain)와 같았다. 이 경우 15:31–15:39 차트값 전부 동일·첫값 15:35 이내 조건도 확인한다. 하나라도 확인되지 않으면 평가에서 제외한다. [KRX 2026 휴장 안내](https://kind.krx.co.kr/external/dst/notice/11637/%5B%ED%95%9C%EA%B5%AD%EA%B1%B0%EB%9E%98%EC%86%8C%5D%202026%EB%85%84%20%EC%98%AC%EB%B9%BC%EB%AF%B8%EA%B3%B5%EC%8B%9C%20%EC%95%88%EB%82%B4.pdf)는 9/24~27 휴장을 확인한다. 미확인 휴장·특수 개장일에는 목표일의 KRX 확정 종가가 없거나 분봉과 다르면 해당 종목을 제외하며, 전체가 미평가이면 게시를 멈춘다. 과거 수정주가 계열의 의미는 추론하지 않는다. — Codx
- [KRX KIND 상장종목현황](https://kind.krx.co.kr/corpgeneral/listedIssueStatus.do?method=loadInitPage)의 2026-09-23 공개 행을 KOSPI 914·KOSDAQ 1,801개에서 확인했다. 보통 삼성전자 `KR7005930003`와 우선 삼성전자우 `KR7005931001`이 별도 행이며, 정규화 seed 285개 중 266개는 공식 상장명·코드가 일치했다(조회 응답 합성 SHA-256 `6788315e71fa610049d8731a0f49f120366856c35ba672f166ec7221c28c86f4`). 공시의 의결권 주식량=전체 주식등량, KRX 상장 종목 이름·코드·ISIN 00계열을 함께 만족할 때만 수량 평가 후보로 승격한다. 나머지 19개와 원문 class 불일치는 미평가로 남긴다. 현재 DART XML 원본을 로컬에서 조회할 키가 없어 실승격 수량은 CI에서 별도 판정한다. — Codx
- `datasetVersion`은 자기 자신을 포함할 수 없으므로 snapshot의 최상위 `datasetVersion`을 빼고 당일 history 행의 `datasetVersion`을 빈 자리표시자로 둔 정규 JSON projection의 SHA-256이다. 최종 JSON 전체 바이트의 해시는 별도 `manifest.snapshotSha256`이며 빌드 검사와 브라우저 loader가 이 바이트 해시를 확인한다. 다일 history는 실제 게시 완료 state와 현 후보의 검증된 평가 1점을 합치고 같은 거래일은 최신 후보로 대체한다. — Codx
...
- [OpenDART 공식 목록 가이드](https://opendart.fss.or.kr/guide/detail.do?apiGrpCd=DS001&apiId=2019001)의 `rm=정`(후속 정정), `rm=철`(철회)와 보고서명 `[정정]`은 접수번호별 상태로 보존한다. 이 응답만으로 원접수번호와 정정본의 연결을 확정할 수 없어 `correction_of=null`과 미해결 큐를 유지하며 최신 보유를 덮지 않는다. 같은 날 접수번호 순서와 겹침 재실행, 구조화 응답 지연 시 정확한 접수번호의 XML fallback은 합성 단위 검사로 확인했다. 실제 정정 연결과 취소 반영은 live 미실행이다. — Codx
...
- **감사기 자체 결함 선행 해소:** `tests/accessibility-audit.mjs`가 루트 `/`를 ko-KR로 가정했으나 제품 기본 언어는 `src/i18n/languages.ts`의 `en`이라 **첫 페이지에서 감사가 중단**됐다. 이제 그 선언을 읽어 단일 출처로 쓰고, 선언을 찾지 못하면 조용히 넘어가지 않고 실패한다. 이 수정 없이는 접근성 검사가 계속 헛돈다.
- **회색 토큰 — 일괄 하향을 기각했다:** `--label-tertiary`(`#909098`)를 4.5:1까지 올리면 `#606068`이 되어 `--label-secondary`와 같아지고 연한 회색 두 단계의 위계가 사라진다. 그래서 토큰을 일괄로 내리지 않고, 라이트 2테마의 `--label-secondary`만 `#69696f`→`#636369`(4.13→4.52)로 내리고 **실측에서 미달인 tertiary 소비자 4곳**(`.nav-caption`, kbd 2종, `.global-footer`, `.tool-category-heading b`)만 secondary로 교체했다. tertiary 토큰 값은 유지해 3단계 위계(`#18181b` > `#636369` > `#909098`)가 남는다. 다크 테마는 7.48:1·5.59:1로 통과하므로 건드리지 않았다.
- **문서 비교 diff는 색 문제가 아니었다:** 흰 배경에서 파랑 7.70:1·빨강 5.68:1로 통과한다. 보고된 2.31:1·1.62:1은 **320px에서 종류 배지(absolute 오버레이)가 diff 텍스트를 덮어** 나온 값이었다. 따라서 색값을 바꾸지 않고 배지만 `max-[820px]`에서 본문 위로 흐르게 했다. 파랑=삭제·빨강=추가 의미와 데스크톱 렌더링은 불변이다. 색을 진하게 하는 해법은 원인이 아니므로 기각했다.
- **`region` 57노드:** 전부 상단바 밖 자식(github 33·select 15·switcher 9)이었다. `.wl-topbar`에 `role="region"`과 한영 `aria-label`을 부여해 해소했고 시각 변화는 없다.
- **실측 전후(46페이지):** 자동 위반 **64 → 28**(serious 31·moderate 33 → serious 28·moderate 0), `region` 노드 **57 → 0**, `color-contrast` 위반 노드 **89 → 72**(kbd 17 해소·신규 0), incomplete **1,858 → 1,807**, PDF 스탬프 모바일 안내문 **4.4179 → 4.8341**(게이트 통과). 종료 코드 1은 남는데 잔류 72노드는 회색 범위 밖과 기각된 브랜드 incomplete다.
- **총괄 독립 확인:** 코랄·다크 토큰 값 불변, 최종 토큰 3단계 위계, 상단바 `role=region`·라벨 실재, `.nav-caption`·`.global-footer` 실측 5.49:1, 사이드바 캡처 육안으로 위계 보존 확인.
- 시각 회귀 274/274, 기준 이미지 228장 갱신(영향은 전 페이지 푸터·캡션), 재검증에서 해당 귀속 픽셀 실패 0. 잔류 5건(video-studio 3·office-editor 2)은 기존 항목이며 통과로 처리하지 않았다. — Claude 판정(Opus) / Muse 구현 / Codx 선행 판정

### 시각 회귀 스위트 복구와 UI v3 게이트 구조 항목 (Claude 판정)

- **왜 필요했나:** 기준 이미지 마지막 갱신이 `c00f3d1`(2026-09-15)인데 그 뒤 `src/`가 86파일 9,283줄 바뀌어 **266/266이 전부 실패**했다. 즉 시각 회귀가 어떤 UI 변경도 검증하지 못하는 상태였다. 사용자가 2026-09-21 재생성을 명시 승인했다.
- **무조건 갱신하지 않았다:** 갱신 전 표본 12건의 `.actual.png`와 `.diff.png`를 직접 열어 전부 승인된 UI 변경이 반영된 정상 화면임을 확인한 뒤에만 갱신했다. 깨진 화면이 하나라도 있으면 중단하는 조건을 지시서에 넣었다. 결과 249장 갱신·신규 0·고아 0, 갱신 없는 재검증에서 픽셀 실패 0. 커밋 `7e63b0e`에 PNG 외 파일 0건.
- **검사가 헛돌던 것을 발견했다:** 잔류 실패 17건 중 14건은 화면이 아니라 하네스 셀렉터 노후였다. home은 `.hero`→`.wl-hero`, office-editor는 랜딩이 `/app`으로 대체되는 구조를 반영하지 못해 `[data-tool-page='office-editor']`가 잡히지 않았다. 즉 그 14개 시나리오는 **아무것도 검증하지 못하고 있었다**. `tests/rendering-baseline.mjs`의 exit 1도 같은 원인이었다. 커밋 `4cf798d`으로 해소.
- **desktop-1920:** 전 시나리오 곱셈을 기각하고 hero·index grid·문서 rail·미리보기 rail 4개에만 KO/light 1프로파일씩 적용했다(+4장). 1920 폭에서 rail 겹침·글자 세로 낙하가 없음을 생성 전 육안 확인했다. 커밋 `5635d24`.
- **mint family:** `themeForProfile`이 light/dark-mint를 자기 자신으로 해소하고 러너의 coral 하드코딩을 제거했다. `dark-mint` 거부를 기대하던 unit 단언은 기대치를 낮춘 것이 아니라 새 계약에 맞춰 명시 단언으로 갱신했다(+4장). 커밋 `29a5675`.
- **VISUAL_SHARD:** `VISUAL_SHARD=k/n` 연속 슬라이스로 구현하고 UPDATE 병용을 금지하는 가드를 넣었다. 러너와 self-check가 같은 매트릭스 모듈을 공유해 러너가 스스로 속지 않는다. 총괄 독립 재실행 결과 **N=274, shard 137/137, 합집합 274, 교집합 0, 누락 0, 중복 0, 순서 안정**. 커밋 `75c246c`.
- **N=274 실측:** 266 + desktop-1920 4 + mint 4. viewport별 desktop 125 / desktop-1920 4 / mobile 136 / mobile-320 9. 정본의 과거 숫자(183×2, 476)를 복사하지 않고 실제로 셌다.
- **남은 실패 5건은 통과로 적지 않는다:** video-studio navigation timeout 3건(하네스 조건, 별도 진단 완료)과 office-editor 진행률 flake 2건이다. actual 확인 결과 화면 자체는 정상이다.
- 네 커밋 모두 `src/` 변경 0건으로 제품 동작에 영향이 없다. — Claude 판정(Opus) / Muse 구현

### 접근성 incomplete 판정과 현행 게이트 실패 (Claude 판정)

- **2,113은 확정 결함 수가 아니었다:** 2026-09-15의 incomplete 2,113 노드는 axe-core가 배경 gradient·겹침·pseudo element를 해석하지 못해 사람 판단으로 넘긴 항목이다. 현행 HEAD에서 1,859 노드를 실제 렌더 픽셀 대비로 판정한 결과 **실제 결함 514 / 결함 아님 1,333 / 판정 불가 12**다.
- **별개로 현행 게이트가 실패한다:** 자동 위반이 2026-09-15의 0에서 **64건**(serious 31·moderate 33, 146 노드)으로 늘었다. `color-contrast` 89 노드와 `region` 57 노드다. 이것은 판정 대상이 아니라 확정 위반이다.
- **결함은 네 군데 색 문제가 화면마다 반복된 것이다:** ① `--label-secondary` `#69696f`가 밝은 카드 배경에서 4.13:1(필요 4.5) ② 브랜드 카드 흰 글씨 on 코랄 3.32:1, `.brand-tagline` 2.87:1 ③ 문서 비교 diff 파란 삭제 2.31:1·빨간 추가 1.62:1(320px 겹침) ④ 단계 번호 배지 흰 글씨. `region` 57건은 화면 변화 없는 랜드마크 구조 문제다.
- **수정 규모 차이:** ①은 `#636369`로 내리면 4.52:1로 해소되며 256단계 중 6단계라 육안 차이가 사실상 없다. ②는 흰 글씨 유지 시 코랄을 `rgb(184,80,58)`까지 내려야 4.95:1이 되어 **브랜드 색이 눈에 띄게 바뀐다**. 승인된 테마 변경이므로 총괄이 임의로 결정하지 않고 사용자 판단으로 넘긴다.
- **감사기 자체 결함:** `tests/accessibility-audit.mjs`의 `home` 등록이 `/`를 ko-KR로 추정하지만 현행 기본 언어는 `src/i18n/languages.ts`의 `en`이다. 이 불일치로 **첫 페이지에서 감사가 중단된다**. 구현자가 실행 시 메모리 주입으로 우회해 46페이지를 완주했고, 이것이 이번 변경 때문이 아니라 선행 결함임을 pristine 대조로 실증했다. 고치지 않으면 접근성 검사가 계속 헛돈다. — Claude 판정(Opus) / Codx 판정
...
- **S4 계약 갱신:** S5 수정으로 재로드 패턴이 바뀌었으므로 S4의 기대값을 새로운 계약에 맞춰 갱신했다. 단언: 재로드 개수 `chunk404 >= 2`(복구 예산 유지), 도착 문서 광고 script 0. — Claude

- **해소 — S9 광고 제외 경로 문서 교체:** 근본 원인은 앱이 등록한 `beforeunload` 리스너가 사용자가 앱 확인창에서 이미 승인한 동일 이동까지 다시 차단한 것이며, 테스트의 일괄 dismiss가 그 원생 확인창을 취소해 문서 교체가 무산되고 이전 문서의 광고 script가 남았다. SPA 분기 가설은 `history.back` 0회로 배제했다. 승인 intent(출발 URL·작업 세대)에 한정해 차단을 건너뛰고 승인 없는 새로고침·닫기 보호는 유지한다. 동일 조건 대조 실측은 수정 전 원생 dialog 1·문서 교체 0·광고 script 1, 수정 후 원생 dialog 0·문서 교체 1·광고 script 0이다. — Claude 판정(Opus) / Codx 확인

- **S10 모바일 overlay 범위 제외:** 사용자 판단에 따라 이번 배포 범위에서 제외. — Claude

### AdSense 배포 일정 조정

다이얼로그·toolState·도구 연결 등 S9 기반 구현은 `work/adsense-s9-muse-20260921` 브랜치에 보존하며 다음 재개 시 활용할 수 있도록 둔다. 근본 원인 규명 미해결이므로 검증이 복합하고, 화면 테스트와 실제 도구 동작이 분리되어 있다는 점에서 별도 재검토가 필요하다.
...
- **unit 실패 10→4 귀속 갱신 (Claude 판정):** 통합 후보에서 전체 605건을 끝까지 실행해 601건 통과·4건 실패를 확인했다. 확장자 없는 import 3건은 `.ts` 확장자를 명시해 실제 하위 테스트가 실행되도록 수리했다. SEO title 기대값은 승인 카피 현행값으로, FAQ 개수는 실제 개수(5·4)로 갱신하고 KO/EN 동수 검사를 추가했다. `feature-locales`는 features 단언을 유지하고 가이드 내용 검사(중복 키·독립 목록·자동 연결 부정)를 `guides.json` 실제 선택 FAQ로 옮겼으며 KO 부정 구절 단언을 Astra 검수 뒤 보강했다. 해소된 6건 외 잔여는 이름이 같은 `p1b-components` 3건과 `ui-legacy-isolation` 1건이며 `adsense-followup2` 계획으로 이월한다. — Muse
- **광고 스모크 판별·메타데이터 정정:** 고정 빌드 출처 문구를 제거하고 실행 시점의 `runHead`와 `distMtime`을 분리 기록한다. 판별 모드는 `AssertionError`·`ERR_ASSERTION`·정확한 스텁 메시지를 모두 확인하고, S5와 판별 경로의 네트워크 단언을 별도로 실행하며 실패 결과에도 4계수를 보존한다. 통합 실행은 `runHead=c942a37`, fail 0, 전 시나리오 `allowedExternal=0`, 판별 성공이었다. — Muse
- **S5 같은 문서 오류와 잔류 위험:** 테스트 전용 렌더 오류 주입으로 같은 문서 전환을 재현하고 기존 광고 스크립트 태그 잔류를 실측했다. 이는 주입 재현이며, 순수 렌더 오류 시 잔류 광고 처리 여부는 결정이 필요하다. Claude 판정으로 이번 범위 밖·백로그에 둔다. — Muse
- **S6 PDF 경로군 전환:** `/tools/pdf-editor/merge` 정확 링크 부재는 `S6-ko-merge`의 적용 대상 아님으로 분리했다. 실제 NavLink인 `/tools/pdf-editor` 루트로 한국어·영어 전환을 검사해 각각 새 문서 1회, 전환 뒤 광고 태그 0·추가 스텁 0을 확인했다. — Muse
...
규칙 19의 추적 없는 QA에서 Gemini 실제 브라우저 80환경, Claude 육안, Codx DOM 교차를 수행했다. 기존 선택 191경로/164고유 이미지 중 Claude CLI **82개**, agy Opus **50개**, Codex Astra 보충 **23개**를 실제 이미지 도구로 열람했다. 나머지 9개는 수리 전 도장 초기 화면이며, 이미 열람한 같은 종류 7개와 함께 **16개 초기 화면 전부를 수리 후 Opus 검수로 교체**했다. 그 16개와 변경된 도장 interaction 3개를 합친 Opus **19장**은 별도 원도구 이벤트·SHA·개별 소견으로 확인했다. 동일 SHA 별칭은 별도 열람으로 세지 않았고, EN desktop 기준선 2개의 추가 바이트 차이는 pixelmatch 차이 0으로 기존 검수에 연결했다. 최종 대조 `/tmp/worklazy-u4-mergegate4/visual-closure.json`의 미확인 필수 상태와 미해결 신규 P2는 **0**이다.

초기 generic Gemini 보고서와 한도 중단 작업의 미열람 부분은 승인 근거에서 제외했다. Gemini·Claude CLI에 이어 agy Opus도 실제 429로 중단됐고, Flash 재시도는 429로 열람 0이었다. 마지막 23장은 **Codex Astra 보충 검수**로 명시했으며 Gemini/Claude 전수 완료라고 대필하지 않았다. 기존 고정 shell·언어 선택 가림과 U4 경미한 줄바꿈은 앞선 DOM 도달성 대조 및 backlog에 연결한다. 기존 heartbeat 목표 false, 공용 incomplete 1,973건, Dolby Vision base-layer 미지원 skip, 실기기 모바일 미교정은 여전히 미달·한계로 보존한다. 배포 전 검수는 종결했고 main 병합·한 번의 push·Pages 및 라이브 사후 확인은 이어서 실행한다. — Codx
...
## 2026-09-13 — U6 문서 개인정보 가리기 통합 판정 (Codx)

- P0의 고정 PDF.js factory·worker·ZIP reader/output 소유 장부를 채택했다. 이는 계상한 소유 자원의 경계이며 전체 heap이나 실기기 모바일 메모리 상한 보장은 아니다. P1은 PDF/PNG 출력의 허용 구조·검정 픽셀·메타데이터 제거와 취소·한도 음성을 실제 합성 fixture로 통과했다. P2는 다중 입력, 좌표·DPR2, undo/redo, 무영역 목록, 순차 처리, 이전 결과·ZIP 보존과 URL 정리를 실제 다운로드·재열기로 확인했다.
- P3 전용 정적 문서는 AdSense 계정 메타와 web app manifest를 제거하고 strict CSP·document marker를 둔다. 광고·분석 loader와 SW 등록은 mount/call 0이며, 정상 처리 관측 구간에서 입력 활성화 이후 HTTP(S)·WS 시도 0과 사용자 정보 canary의 storage·console 출현 0을 실제 worker·BFCache·locale 경계에서 확인했다. 공통 정적 검사가 이 두 marker를 다시 요구하던 모순은 전용 페이지의 부재를 명시 검증하도록 고쳤다.
- 최종 등록 bundle graph는 canonical baseline `4caaa9c6…`, 기본 5상한 null, overrides `{}`, multiplier 1에서 exit 0이었다. 실측 gzip은 entry 318,324B, affected route 195,494B, shared 1,944,192B, app 6,143,527B, CSS 39,173B이다. 상한 해제는 계측·inventory 실패 면제로 사용하지 않았다.
- 접근성 최초 raw에서 대비 위반이 39회 발생했으며, 그중 Undo/Remove 2건은 250ms 뒤 같은 DOM에서 위반 0이 되어 transition임을 확인했다. 나머지는 안정 상태 probe로 분리 판정했다. 실제 안정 결함인 preview/log scroll focus, cancel·손상 안내 대비를 수리한 뒤 최종 60상태에서 U6 직접 위반 0, overflow 0, 최대 CLS 0.027803이었다. 남은 desktop LanguageSwitcher 대비 1건과 EN320 전역 consent 버튼 잘림은 공용 부모와 동일해 backlog로 이관했다.
- 최종 broad browser 첫 실행은 Word 비교의 generic error로 중단됐으나, 동일 final dist의 Word 단독·관측·앞선 Excel→Word 동일 context가 모두 exit 0이고 worker/Pyodide 자산 SHA도 같았다. 원인은 미확정 transient로 보존하며 기존/new 어느 쪽으로도 단정하지 않는다. 근거: `/tmp/worklazy-u6-final-word-diagnostic/REPORT.md`.
- 전송 API 소스 감사는 **470개 source file / 167개 exact matched path**와 생성 public 130개를 대조했다. P3가 직접 바꾸지 않은 utilities·Office·legacy 엔진/호출자는 파일 SHA와 기존 단계별 증거를 재사용했고, 직접 영향이 있는 QR·Excel C3 소비자, PDF browser, registry/static graph와 공용 shell 대표 경로만 재실행했다. 잘못된 4350 산출물의 접근성 43건은 무효이며, U6 관련 범위는 정확한 final dist의 60상태·독립 UI 검수·공용 keyboard 표본으로 회수했다.

## 2026-09-13 — U7 DOCX 문서 일괄 생성 통합 판정 (Codx)

- 고정 Core 3.69.3·PizZip 3.2.0 후보는 split run, 표·header/footer·관계·비수정 파트와 XML/LibreOffice 재개방을 통과했다. 제품 코어는 C1 표시값·실제 오류·수식 캐시, 물리 열 별칭, NFC/C2 이름 충돌, 단일 worker, OPFS/200MiB 메모리 장부, 행별 commit과 C3 ZIP·C4 XLSX manifest를 연결했다. 독립 검수에서 core 19파일 해시 불일치 0과 취소·지연 저장·부분 결과·2 DOCX oracle을 확인했다.
- UI는 첫 새 결과 commit 전에는 이전 결과와 ZIP을 유지하고, commit 시 React가 이전 링크를 제거한 뒤 저장소와 URL을 정리한다. 독립 반례로 확인한 선택 행 변경 뒤 stale sample과 이전 ZIP anchor revoke 순서를 수리했고 dist4 재검수에서 두 반례 모두 exit 0이었다. 최종 합성 UI 하네스는 19개 흐름, 9개 화면, 실제 DOCX 2개·개별 결과·6-entry ZIP·두 XLSX 보고서 재개방, 외부 요청·페이지·console 오류 0을 기록했다. 초기 CLS는 모든 표본 0이며 전체 상호작용 수명 합계는 CLS session-window와 구분해 원자료로만 보존했다.
- `@xmldom/xmldom` 0.9.12 직접 소비자 검수에서 ExifReader Node XMP 결과와 image privacy worker의 합성 JPEG 정리 출력은 이전 0.9.11과 동일했다. worker에 DOMParser와 Node require가 없어 XMP 객체가 비는 동작은 기존 한계이며, 두 metadata marker 제거만 양쪽에서 확인했다. 최종 Vite도 변경 없는 `vite.config.ts`에서 같은 ExifReader 4.42.0 browser/module 소스(`src/exif-reader.js`, `src/dom-parser.js`)와 불변인 제품 worker를 해석해 `image-privacy.worker-COYs5HqS.js`를 생성했다. 전 도구 메타데이터 보장으로 확대하지 않는다.
- 최종 owner prebuild·고지 생성, production build 75페이지, static, registry 22, unit 538, Excel 비교·정리, QR, utilities와 recovery 155건을 통과했다. unit 첫 실행의 3건과 utilities 앞선 실행은 새 도구 등록 뒤 남은 목록 개수 단언으로 원 실패를 보존하고 실제 KO 22·모바일 홈 포함 23·EN HWP 제외 21 inventory에 맞춰 수리했다. 정상 analytics loader 양성은 script DOM과 차단된 외부 요청 관측으로 확인했고, 최종 utilities·Excel 비교·정리에서는 외부 HTTP(S)를 각각 128·12·27건 실행 전에 차단했다. 앞선 granted+unblocked 실행은 기능 관측으로만 보존한다. U7 초기·하단 시각 기준선 6장은 생성·재비교에서 U7 diff 0이었으나 명령 전체는 부모 U6 기준선 7장 부재로 exit 1이므로 U7 결과와 기존 전역 부채를 구분한다.
...
- **파일명 제한**: `finish/tokens.ts` 파서·그래핌 축약, `finish/text.ts` 보존 폭맞춤 opt-in(`layoutTextLinesPreserve`, 기존 호출 불변), `finish/engine.ts` 정책 삼상태·검증·전달, `PdfFinishPanel.tsx` 입력·차단·미리보기를 연결했다. 단위 558/558, `test:pdf-finish` 전체 exit 0, 신규 `tests/pdf-filename-limit-smoke.mjs` ko/en 통과.
- **기존 결함(발생 시점: 이번 변경 이전)**: `pdf-finish-smoke.mjs`의 `assertLazyChunks`는 클린 HEAD `9fa435a` 별도 빌드에서도 동일하게 실패한다. Vite가 `?url` 상수를 `pdfRenderLifecycle` 공유 청크로 끌어올려 `PdfEditorPage` 청크 문자열 일치가 성립하지 않으며, 공유 런타임 1개·워커 참조·실제 로딩은 정상이다. 단언을 정적 import 그래프 경유 확인으로 고쳤으며 기대값(단일 공유 런타임·중복 부재)은 낮추지 않았다. 근거: `/tmp/worklazy-baseline-check` 대조 빌드(사용 후 worktree 제거).
- **이월**: U9·파일명 묶음의 main push·Pages·live 확인(배포 승인 필요), U9·파일명 좁은 범위 한·영 선정 화면의 Gemini 실제 열람, UI 재기준화→v3, B1/B2(투자 승인 선행).
